...
public class SecondaryPasswordAuth
{
	...
	public boolean savePassword(String password)
	{
		if(passwordExist())
		{
			_log.atWarning().log( "[SecondaryPasswordAuth]%s forced savePassword", _activeClient.getLogin() );
	...
	public boolean changePassword(String oldPassword, String newPassword)
	{
		if(!passwordExist())
		{
			_log.atWarning().log( "[SecondaryPasswordAuth]%s forced changePassword", _activeClient.getLogin() );
			_activeClient.closeNow();
			return false;
		}
	...
	public boolean checkPassword(String password, boolean skipAuth)
	{
		...
		if(!password.equals(_password))
		{
			...
			else
			{
				...
				GameServer.getInstance().getAuthServerCommunication().sendPacket(new ChangeAccessLevel(_activeClient.getLogin(), accessLvl, banExpire));
				_log.atWarning().log( "%s - (%s) has inputted the wrong password %s times in row.", _activeClient.getLogin(), _activeClient.getIpAddr(), _wrongAttempts );
	...
	private String cryptPassword(String password)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "[SecondaryPasswordAuth] Unsupported Algorythm" );
		}
		catch(UnsupportedEncodingException e)
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "[SecondaryPasswordAuth] Unsupported Encoding" );
		}
		return null;
	}
