...
public class QuestListPacket implements IClientOutgoingPacket
{
	...
	private final Map<Integer, Integer> _quests = new HashMap<>();

	public QuestListPacket(Player player)
	{
		List<QuestState> questsStates = Arrays.stream(player.getAllQuestsStates())
				.sorted(Comparator.comparingInt(qs -> qs.getQuest().getId()))
				.collect(Collectors.toUnmodifiableList());
		for(QuestState quest : questsStates)
		{
	...
	{
		OutgoingPackets.QUEST_LIST.writeId(packetWriter);
		packetWriter.writeH(_quests.size());
		_quests.forEach((id, cond) -> {
			packetWriter.writeD(id);
			packetWriter.writeD(cond);
		});
