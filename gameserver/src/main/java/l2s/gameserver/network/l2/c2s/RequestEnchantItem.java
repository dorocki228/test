...
public class RequestEnchantItem implements IClientIncomingPacket
{
	...
	public void run(l2s.gameserver.network.l2.GameClient client)
	{
		...
		try
		{
			...
			{
				player.sendActionFailed();
				_log.atWarning().log( "RequestEnchantItem: Cannot find variation ID[%s] for enchant scroll ID[%s]!", enchantScroll.getVariationId(), enchantScroll.getItemId() );
				return;
			}
			...
			{
				player.sendActionFailed();
				_log.atWarning().log( "RequestEnchantItem: Cannot find variation ID[%s] enchant level[%s] for enchant scroll ID[%s]!", enchantScroll.getVariationId(), (item.getEnchantLevel() + 1), enchantScroll.getItemId() );
				return;
			}
			...
			if(item.getGrade() != ItemGrade.NONE) {
				chance += player.getStat().getAdd(DoubleStat.ENCHANT_CHANCE_MODIFIER);
				chance *= player.getStat().getMul(DoubleStat.ENCHANT_CHANCE_MODIFIER);
			}
			...
			if(Rnd.chance(chance))
			{
				...
				if(baseChance < 100.) {
					if (item.isArmor() || item.isAccessory()) {
						if (item.getEnchantLevel() >= 9)
							player.broadcastPacket(new ExItemAnnounce(player, item));
			...
			else
			{
				...
				switch(resultType)
				{
					case CRYSTALS:
						...
						ItemLogger.INSTANCE.log(ItemLogger.ItemProcess.EnchantFail, player, item);
