...
public final class RequestReplyStartPledgeWar extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(_answer == 1)
			try
			{
				...
				ClanWar war = activeChar.getClan().getClanWar(clan);
				if(war == null)
				{
				...
				{
					request.cancel();
					activeChar.sendPacket(new SystemMessagePacket(SystemMsg.YOU_HAVE_ALREADY_BEEN_AT_WAR_WITH_THE_S1_CLAN_5_DAYS_MUST_PASS_BEFORE_YOU_CAN_DECLARE_WAR_AGAIN).addString(clan.getName()));
					return;
				}

				war.setPeriod(clan, ClanWar.ClanWarPeriod.MUTUAL);
			}
			finally
			{
				request.done();
	...
	static
	{
		_log = LoggerFactory.getLogger(RequestReplyStartPledgeWar.class);
	}
}
