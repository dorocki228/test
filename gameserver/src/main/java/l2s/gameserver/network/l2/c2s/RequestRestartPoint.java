		...
		switch(restartType)
		{
			...
			case ORIGINAL:
				if(activeChar.getPlayerAccess().ResurectFixed)
					activeChar.doRevive(100);
				else if(activeChar.isAgathionResAvailable())
					activeChar.doRevive(100);
		...
		switch(restartType)
		{
			case AGIT:
				if(clan != null && clan.getHasHideout() != 0)
				{
					...
					teleportPoint = TeleportUtils.getRestartPoint(activeChar, RestartType.AGIT);
			...
			case CASTLE:
				if(clan != null && clan.getCastle() != 0)
				{
					...
					teleportPoint = TeleportUtils.getRestartPoint(activeChar, RestartType.CASTLE);
			...
			case VILLAGE:
			default:
				teleportPoint = TeleportUtils.getRestartPoint(activeChar, RestartType.VILLAGE);
				break;
		}
