...
import l2s.gameserver.utils.loggers.PlayerInfo;
 ...
 *
 * @see RequestPostItemList
 * @see RequestReceivedPostList
 */
public class RequestSendPost implements IClientIncomingPacket
{
	...
	public boolean readImpl(l2s.gameserver.network.l2.GameClient client, l2s.commons.network.PacketReader packet)
	{
		...
		if(_count * 12 + 4 > packet.getReadableBytes() || _count > Short.MAX_VALUE || _count < 1) //TODO [G1ta0] audit
		{
			_count = 0;
			return false;
		}
	...
	public void run(l2s.gameserver.network.l2.GameClient client)
	{
		...
		{
			receiverId = CharacterDAO.getInstance().getObjectIdByName(_recieverName);
			if(receiverId > 0)
				//TODO [G1ta0] корректировать _recieverName
				if(mysql.simple_get_int("target_Id", "character_blocklist", "obj_Id=" + receiverId + " AND target_Id=" + activeChar.getObjectId()) > 0) // цель заблокировала отправителя
				{
					activeChar.sendPacket(new SystemMessagePacket(SystemMsg.C1_HAS_BLOCKED_YOU_YOU_CANNOT_SEND_MAIL_TO_C1).addString(_recieverName));
					return;
				}
		...
		try
		{
			...
			if(_count > 0)
			{
				for(int i = 0; i < _count; i++)
				{
					...
					ItemLogger.INSTANCE.log(ItemLogger.ItemProcess.PostSend, activeChar, PlayerInfo.Companion.findByObjectId(receiverId), item);
