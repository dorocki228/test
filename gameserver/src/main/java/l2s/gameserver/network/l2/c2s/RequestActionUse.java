...
public class RequestActionUse implements IClientIncomingPacket
{
	...
	public static enum Action
	{
        ...
        ACTION92(92, 0, 0), // Next Target
        ACTION93(93, 0, 0), // Next Target
        ACTION94(94, 0, 0), // Next Target
		...
		ACTION78(78, 0, 1), // Использование Знака 1
		ACTION79(79, 0, 1), // Использование Знака 2
		ACTION80(80, 0, 1), // Использование Знака 3
		ACTION81(81, 0, 1), // Использование Знака 4
		ACTION82(82, 0, 1), // Автоприцел Знаком 1
		ACTION83(83, 0, 1), // Автоприцел Знаком 2
		ACTION84(84, 0, 1), // Автоприцел Знаком 3
		ACTION85(85, 0, 1), // Автоприцел Знаком 4
		...
		ACTION1085(1085, 1, 4707), // Shade Sacrifice
		...
		ACTION1123(1123, 2, 14767), // Осадный Удар TODO: Проверить SkillClassId
	...
	public void run(l2s.gameserver.network.l2.GameClient client)
	{
		...
		if(action == null)
		{
			_log.atWarning().log( "unhandled action type %s by player %s", _actionId, activeChar.getName() );
		...
		if(action.type == 0) // Действия с игроками
		{
			...
			if((activeChar.isOutOfControl() || activeChar.isActionsDisabled(action.value == 0)) && !(activeChar.isFakeDeath() && _actionId == 0))
			{
				activeChar.sendActionFailed();
				return;
			}
			...
			switch(action.id)
			{
				...
				case 96: // Quit Party Command Channel?
					_log.atInfo().log( "96 Accessed" );
					break;
				case 97: // Request Party Command Channel Info?
					_log.atInfo().log( "97 Accessed" );
					break;
				case 38: // Mount
				{
		...
		else if(action.type == 1) // Действия петов
		{
			...
			// хозяин в фире не может управлять петами.
		...
		else if(action.type == 2 || action.type == 5) // Действия одного и более саммона
		{
			...
			// хозяин в фире не может управлять саммонами.
			if (activeChar.isOutOfControl()) {
				activeChar.sendActionFailed();
				return;
			}
		...
		else if(action.type == 3) // Социальные действия
		{
			...
			if(Config.ALT_SOCIAL_ACTION_REUSE)
			{
				...
				activeChar.getFlags().setBlockActions(true);
			}
	...
	private boolean servitorUseSkill(Player player, Servitor servitor, int skillId, int actionId)
	{
		...
		Creature aimingTarget = skill.getAimingTarget(servitor, player.getTarget(), skill, _ctrlPressed, _shiftPressed, false);
	...
	static class SocialTask implements Runnable
	{
		...
		public void run()
		{
			_player.getFlags().setBlockActions(false);
		}
	}
