		...
		if(mail != null)
		{
			...
			try
			{
				...
				synchronized (attachments)
				{
					...
					if(!mail.isReturned() && mail.getPrice() > 0)
					{
						...
						if(sender != null)
						{
							...
							ItemLogger.INSTANCE.log(ItemProcess.PostPaymentRecieve, sender, adena, "receive mail payment: message_id[" + mail.getMessageId() + "], receiver_id[" + mail.getReceiverId() + "]");
						}
						else
						{
				...
				for(ItemInstance item : items)
				{
					...
					ItemLogger.INSTANCE.log(safePost ? ItemProcess.SafePostRecieve : ItemProcess.PostRecieve, activeChar, item, "receive mail attachments: message_id[" + mail.getMessageId() + "], sender_id[" + mail.getSenderId() + "]");
