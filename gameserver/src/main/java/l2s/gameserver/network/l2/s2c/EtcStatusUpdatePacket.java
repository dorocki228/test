...
public class EtcStatusUpdatePacket implements IClientOutgoingPacket
{
	...
	{
		_increasedForce = player.getCharges();
		_weightPenalty = player.getWeightPenalty();
		_weaponPenalty = player.getExpertiseWeaponPenalty();
		_armorPenalty = player.getExpertiseArmorPenalty();
	...
	{
		OutgoingPackets.ETC_STATUS_UPDATE.writeId(packetWriter);
		// cdccccc
		packetWriter.writeC(_increasedForce); // skill id 4271, 7 lvl
		packetWriter.writeD(_weightPenalty); // skill id 4270, 4 lvl
		packetWriter.writeC(_weaponPenalty); // weapon grade penalty, skill 6209 in epilogue
		packetWriter.writeC(_armorPenalty); // armor grade penalty, skill 6213 in epilogue
		packetWriter.writeC(0x00); //Death Penalty max lvl 15, "Combat ability is decreased due to death."
		packetWriter.writeC(0x00); //Consumed souls
