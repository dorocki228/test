...
import l2s.gameserver.utils.loggers.MultisellLogger;
	...
	public void run(l2s.gameserver.network.l2.GameClient client)
	{
		...
		try
		{
			...
			for(MultiSellIngredient i : entry.getProduction())
			{
				...
				if(item == null)
				{
					_log.atWarning().log( "Cannot find production item template ID[%s] in multisell list ID[%s]!", i.getItemId(), _listId );
					return;
				}
			...
			for(MultiSellIngredient ingridient : entry.getIngredients())
			{
				...
				else
				{
					...
					if(template == null)
					{
						_log.atWarning().log( "Cannot find ingridient item template ID[%s] in multisell list ID[%s]!", ingridientItemId, _listId );
						return;
					}
			...
			for(MultiSellIngredient in : products)
			{
				...
				else if(ItemHolder.getInstance().getTemplate(in.getItemId()).isStackable())
				{
					...
					MultisellLogger.INSTANCE.log(activeChar, in.getItemId(), in.getItemCount(), total);
				}
				else
				{
					for(int i = 0; i < _amount; i++)
					{
						...
						MultisellLogger.INSTANCE.log(activeChar, in.getItemId(), in.getItemCount(), 1);
					}
				}
			}
