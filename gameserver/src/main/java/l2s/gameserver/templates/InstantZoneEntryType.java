...
public enum InstantZoneEntryType
{
	SOLO
	{
		...
		{
			SystemMsg msg = reCheckPlayer(player, instancedZone);
			if (msg != null)
			{
				if(msg.size() > 0)
					player.sendPacket(new SystemMessagePacket(msg).addName(player));
	...
	PARTY
	{
		...
		public boolean canReEnter(Player player, InstantZone instanceZone)
		{
			...
			SystemMsg msg = reCheckPlayer(player, instanceZone);
			if (msg != null)
			{
				if(msg.size() > 0)
					player.sendPacket(new SystemMessagePacket(msg).addName(player));
	...
	COMMAND_CHANNEL
	{
		...
		public boolean canEnter(Player player, InstantZone instancedZone)
		{
			boolean isCcOnly = instancedZone.getMinParty() > 9;
			Party party = player.getParty();
			if(party == null || (party.getCommandChannel() == null && isCcOnly))
			{
			...
			if((isCcOnly && cc.getMemberCount() < instancedZone.getMinParty()) || !isCcOnly && party.getMemberCount() < instancedZone.getMinParty())
			{
				player.sendPacket(new SystemMessagePacket(SystemMsg.YOU_MUST_HAVE_A_MINIMUM_OF_S1_PEOPLE_TO_ENTER_THIS_INSTANT_ZONE).addInteger(instancedZone.getMinParty()));
				return false;
			}
			if(isCcOnly && cc.getMemberCount() > instancedZone.getMaxParty())
			{
				player.sendPacket(SystemMsg.YOU_CANNOT_ENTER_DUE_TO_THE_PARTY_HAVING_EXCEEDED_THE_LIMIT);
				return false;
			}

			PlayerGroup plyrs = party.getCommandChannel() == null ? party : cc;
			for(Player member : plyrs)
			{
				if(!player.isInRange(member, 500))
				{
					plyrs.broadCast(new SystemMessagePacket(SystemMsg.C1_IS_IN_A_LOCATION_WHICH_CANNOT_BE_ENTERED_THEREFORE_IT_CANNOT_BE_PROCESSED).addName(member));
					return false;
				}
				...
				{
					if(msg.size() > 0)
						plyrs.broadCast(new SystemMessagePacket(msg).addName(member));
					else
						member.sendPacket(msg);
					return false;
				}
		...
		public boolean canReEnter(Player player, InstantZone instanceZone)
		{
			boolean isCcOnly = instanceZone.getMinParty() > 9;
			Party commparty = player.getParty();
			if(commparty == null || (commparty.getCommandChannel() == null && isCcOnly))
			{
				player.sendPacket(SystemMsg.YOU_CANNOT_ENTER_BECAUSE_YOU_ARE_NOT_ASSOCIATED_WITH_THE_CURRENT_COMMAND_CHANNEL);
				return false;
			}
			CommandChannel cc = commparty.getCommandChannel();
			if(isCcOnly && cc.getMemberCount() > instanceZone.getMaxParty())
			{
				player.sendPacket(SystemMsg.YOU_CANNOT_ENTER_DUE_TO_THE_PARTY_HAVING_EXCEEDED_THE_LIMIT);
				return false;
			}
