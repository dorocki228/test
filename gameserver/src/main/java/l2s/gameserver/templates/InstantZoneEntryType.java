...
public enum InstantZoneEntryType
{
	...
	PARTY
	{
		...
		public boolean canEnter(Player player, InstantZone instancedZone)
		{
			...
			{
				player.sendPacket(new SystemMessagePacket(SystemMsg.YOU_MUST_HAVE_A_MINIMUM_OF_S1_PEOPLE_TO_ENTER_THIS_INSTANT_ZONE).addNumber(instancedZone.getMinParty()));
				return false;
			}
			...
			if(HwidUtils.INSTANCE.haveSameHwids(party.getPartyMembers()))
			{
				for(Player member : party.getPartyMembers())
				{
					var message = member.isLangRus()
								  ? "Вы не можете зайти в инстанс зону несколькими персонажами с одного HWID."
								  :"You can not enter a dungeon zone with several characters from one HWID.";
	...
	public abstract boolean canEnter(Player p0, InstantZone p1);

	public abstract boolean canReEnter(Player p0, InstantZone p1);
	...
	private static SystemMsg checkPlayer(Player player, InstantZone instancedZone)
	{
		...
		if(!player.hasClubCard() || instancedZone.getId() != Constants.CRUTCH_INSTANCE_ID) {
