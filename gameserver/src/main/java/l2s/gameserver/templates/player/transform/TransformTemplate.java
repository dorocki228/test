...
public class TransformTemplate extends PCTemplate
{
	...
	private final TIntObjectMap<BaseStatsBonus> _baseStatsBonuses;
	private final TIntDoubleMap _levelBonusData;
	private final TIntObjectMap<HpMpCpData> _hpMpCpData;
	private final TIntSet _actions;
	private final List<SkillLearn> _skills;
	private final List<SkillLearn> _additionalSkills;
	private LockType _itemCheckType;
	private final TIntSet _itemCheckIDs;

	public TransformTemplate(StatsSet set)
	{
		super(set);
		_baseStatsBonuses = new TIntObjectHashMap<>();
		_levelBonusData = new TIntDoubleHashMap();
		_hpMpCpData = new TIntObjectHashMap<>();
		_actions = new TIntHashSet();
		_skills = new ArrayList<>();
		_additionalSkills = new ArrayList<>();
		_itemCheckType = LockType.NONE;
		_itemCheckIDs = new TIntHashSet();
		_id = set.getInteger("id");
		_type = (TransformType) set.getEnum("type", (Class) TransformType.class, (Enum) TransformType.COMBAT);
	...
	{
		return _skills.toArray(new SkillLearn[0]);
	}
	...
	{
		return _additionalSkills.toArray(new SkillLearn[0]);
	}
