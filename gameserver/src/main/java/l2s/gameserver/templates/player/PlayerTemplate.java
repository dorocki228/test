...
public final class PlayerTemplate extends PCTemplate
{
	...
	{
		super(set);
		_startLocs = new ArrayList<>();
		_startItems = new ArrayList<>();
		_race = race;
		_sex = sex;
		...
		_maxINT = set.getInteger("maxINT", 100);
		_maxSTR = set.getInteger("maxSTR", 100);
		_maxCON = set.getInteger("maxCON", 100);
		_maxMEN = set.getInteger("maxMEN", 100);
		_maxDEX = set.getInteger("maxDEX", 100);
		_maxWIT = set.getInteger("maxWIT", 100);
	...
	{
		return _startItems.toArray(new StartItem[0]);
	}
