...
public class ZoneTemplate
{
	...
	private final SystemMsg _enteringMessage;
	private final SystemMsg _leavingMessage;
	 ...
	 */
	private final SystemMsg _damageMessage;
    ...
    private final double _moveBonusMul;
    ...
    private final List<GveOutpost> outposts;
	...
	public ZoneTemplate(StatsSet set)
	{
        ...
        inGameName = set.getString("in_game_name", _name);
		_type = ZoneType.valueOf(set.getString("type"));
		_territory = (Territory) set.get("territory");

		int id;
		_enteringMessage = (id = set.getInteger("entering_message_no", -1)) == -1 ? null : SystemMsg.valueOf(id);
		_leavingMessage = (id = set.getInteger("leaving_message_no", -1)) == -1 ? null : SystemMsg.valueOf(id);
        ...
        _moveBonusMul = set.getDouble("move_bonus_mul", 0.0);
		...
		_damageMessage = (id = set.getInteger("message_no", -1)) == -1 ? null : SystemMsg.valueOf(id);
		...
		if(s != null)
			_blockedActions = s.split(PropertiesParser.defaultDelimiter);
		else
		...
		outposts = (List<GveOutpost>) set.getOrDefault("outposts", List.of());
	...
	{
		return _enteringMessage;
	}

	public SystemMsg getLeavingMessage()
	{
		return _leavingMessage;
	}
