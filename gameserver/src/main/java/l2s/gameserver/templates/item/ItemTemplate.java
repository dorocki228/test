...
public abstract class ItemTemplate extends StatTemplate
{
	...
	protected final Map<DoubleStat, Optional<Double>> baseValues = new EnumMap<>(DoubleStat.class);

	private final AgathionTemplate _agathionTemplate;

	private final boolean autousable;
	...
	protected ItemTemplate(final StatsSet set)
	{
		...
		setAttachedFuncs(FuncTemplate.EMPTY_ARRAY);
	...
	public int getBaseAttributeValue(AttributeType attributeType)
	{
		if(attributeType == AttributeType.NONE)
			return 0;
		return _baseAttributes[attributeType.getId()];
	}
	...
	public boolean isBlocked(Playable playable, ItemInstance instance)
	{
		...
		for(Condition condition : _conditions)
		{
			if(!condition.test(playable, null, null, instance, 0) && condition.getSystemMsg() == null)
				return true;
		}
		return false;
	}
	...
	public void attachFunc(FuncTemplate f)
	{
		...
		if(isWeapon())
		{
			if(f._stat == DoubleStat.PHYSICAL_ATTACK && f._func == FuncAdd.class && f._order == 0x10)
			{
				baseValues.put(DoubleStat.PHYSICAL_ATTACK, Optional.of(f._value));
				return;
			}
			if(f._stat == DoubleStat.MAGICAL_ATTACK && f._func == FuncAdd.class && f._order == 0x10)
			{
				baseValues.put(DoubleStat.MAGICAL_ATTACK, Optional.of(f._value));
				return;
			}
			if(f._stat == DoubleStat.ACCURACY_COMBAT && f._func == FuncAdd.class && f._order == 0x10)
			{
				baseValues.put(DoubleStat.ACCURACY_COMBAT, Optional.of(f._value));
				return;
			}
			if (f._stat == DoubleStat.EVASION_RATE && f._func == FuncAdd.class && f._order == 0x10)
			{
				baseValues.put(DoubleStat.EVASION_RATE, Optional.of(f._value));
				return;
			}
			if (f._stat == DoubleStat.PHYSICAL_ATTACK_RANGE && f._func == FuncAdd.class && f._order == 0x40)
			{
				baseValues.put(DoubleStat.PHYSICAL_ATTACK_RANGE, Optional.of(f._value));
				return;
			}
		}
		else if(isArmor())
		{
			switch(_exType)
			{
				...
				case PENDANT:
				case CLOAK:
					if(f._stat == DoubleStat.PHYSICAL_DEFENCE && f._func == FuncAdd.class && f._order == 0x10)
					{
						baseValues.put(DoubleStat.PHYSICAL_DEFENCE, Optional.of(f._value));
						return;
					}
					break;
			}

			if(f._stat == DoubleStat.MAX_MP && f._func == FuncAdd.class)
			{
				baseValues.put(DoubleStat.MAX_MP, Optional.of(f._value));
				return;
			}
		}
		else if(isAccessory())
		{
			switch(_exType)
			{
				...
				case EARRING:
				case NECKLACE:
					if(f._stat == DoubleStat.MAGICAL_DEFENCE && f._func == FuncAdd.class && f._order == 0x10)
					{
						baseValues.put(DoubleStat.MAGICAL_DEFENCE, Optional.of(f._value));
						return;
					}
					break;
			}
	...
	public Optional<Double> getStat(DoubleStat stat) {
