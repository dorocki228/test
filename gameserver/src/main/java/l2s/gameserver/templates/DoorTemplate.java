...
public class DoorTemplate extends CreatureTemplate
{
	...
	private void setAI(String ai)
	{
		...
		if(classAI == null)
			_log.atSevere().log( "Not found ai class for ai: %s. DoorId: %s", ai, _id );
		else
		{
			_classAI = classAI;
		...
		if(_classAI.isAnnotationPresent(Deprecated.class))
			_log.atSevere().log( "Ai type: %s, is deprecated. DoorId: %s", ai, _id );
	}
	...
	public CharacterAI getNewAI(DoorInstance door)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "Unable to create ai of doorId %s", _id );
		}
