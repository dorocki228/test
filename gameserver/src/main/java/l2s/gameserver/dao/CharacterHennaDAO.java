...
public class CharacterHennaDAO
{
	...
	public List<Henna> select(Player owner)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "CharacterHennaDAO.select(Player): %s", e );
		}
		finally
		{
	...
	public boolean insert(Player owner, Henna henna)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s could not add henna to henna list: %s", owner.getHennaList(), henna );
			return false;
		}
		finally
		{
	...
	private boolean delete(Player owner, int symbolId, int drawTime, boolean premium)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s could not delete henna: %s ownerId: %s", owner.getHennaList(), Henna.toString(symbolId, drawTime, premium), owner.getObjectId() );
			return false;
		}
		finally
		{
