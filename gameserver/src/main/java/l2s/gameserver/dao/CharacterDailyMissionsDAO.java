...
public class CharacterDailyMissionsDAO
{
	...
	public void restore(Player owner, Map<Integer, DailyMission> map)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "CharacterDailyMissionsDAO.select(Player): %s", e );
		}
		finally
		{
	...
	public boolean store(Player owner, Collection<DailyMission> missions)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s could not store missions list: ", owner.getDailyMissionList() );
			return false;
		}
		finally
		{
	...
	public boolean insert(Player owner, DailyMission mission)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s could not insert mission to missions list: %s", owner.getDailyMissionList(), mission );
			return false;
		}
		finally
		{
	...
	private boolean delete(Player owner, int missionId)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s could not delete mission: OWNER_ID[%s], MISSION_ID[%s]:", owner.getDailyMissionList(), owner.getObjectId(), missionId );
			return false;
		}
		finally
		{
