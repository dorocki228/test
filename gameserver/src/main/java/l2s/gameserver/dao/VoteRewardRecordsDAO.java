...
public class VoteRewardRecordsDAO {
	...
	public void restore(Map<String, VoteRewardRecord> records, String site) {
		...
		{
			LOGGER.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "VoteRewardRecordsDAO.select(String):%s", e );
		}
		finally
		{
	...
	private void save0(VoteRewardRecord voteRewardRecord) {
		...
		try  {
			...
			LOGGER.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "VoteRewardRecordsDAO.save0(VoteRewardRecord):%s", e );
		}
		finally {
	...
	private void update0(VoteRewardRecord voteRewardRecord) {
		...
		try  {
			...
			LOGGER.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "VoteRewardRecordsDAO.update0(VoteRewardRecord):%s", e );
		}
		finally {
			DbUtils.closeQuietly(con, ps);
		}
