...
public class OlympiadHistoryDAO
{
	...
	public Map<Boolean, List<OlympiadHistory>> select()
	{
		...
		try
		{
			...
			rset = statement.executeQuery("SELECT * FROM olympiad_history ORDER BY game_start_time");
			map = new HashMap(2);
	...
	public void insert(OlympiadHistory history)
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("INSERT INTO olympiad_history(object_id_1, object_id_2, class_id_1, class_id_2, name_1, name_2, game_start_time, game_time, game_status, game_type, old) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
	...
	public void switchData()
	{
		...
		try
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.createStatement();
			statement.execute("DELETE FROM olympiad_history WHERE old=1");
			DbUtils.close(statement);
			statement = con.createStatement();
			statement.execute("UPDATE olympiad_history SET old=1");
		}
		catch(Exception e)
		{
			_log.error("OlympiadHistoryDAO: select(): " + e, e);
