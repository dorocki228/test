...
public class CharacterPremiumItemsDAO
{
	...
	public List<PremiumItem> select(Player owner)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "CharacterPremiumItemsDAO.select(L2Player): %s", e );
		}
		finally
		{
	...
	public boolean delete(Player owner, PremiumItem item)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s could not delete item: %s ownerId: %s", owner.getBlockList(), item, owner.getObjectId() );
			return false;
		}
		finally
		{
	...
	public boolean update(Player owner, PremiumItem item, long count)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s could not update item: %s", owner.getBlockList(), item );
			return false;
		}
		finally
		{
