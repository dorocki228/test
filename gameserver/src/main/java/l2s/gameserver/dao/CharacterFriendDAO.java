...
public class CharacterFriendDAO
{
	...
	public TIntObjectMap<Friend> select(Player owner)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "CharacterFriendDAO.load(L2Player): %s", e );
		}
		finally
		{
	...
	public void insert(Player owner, Player friend)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s could not add friend objectid: %s", owner.getFriendList(), friend.getObjectId() );
		}
		finally
		{
	...
	public boolean updateMemo(Player owner, int friend, String memo)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s could not update memo objectid: %s", owner.getFriendList(), friend );
			return false;
		}
		finally
		{
	...
	public void delete(int ownerId, int friendId)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s: could not delete friend objectId: %s ownerId: %s", getClass().getSimpleName(), friendId, ownerId );
		}
		finally
		{
			DbUtils.closeQuietly(con, statement);
