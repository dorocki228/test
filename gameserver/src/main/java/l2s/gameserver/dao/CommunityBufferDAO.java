...
public class CommunityBufferDAO {
	...
	public Map<Integer, BuffSet> restore(int ownerId) {
		synchronized (CACHED_BUFF_SETS) {
			...
			try {
				...
				LOGGER.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "CharacterSubclassDAO:restore(int)" );
			} finally {
	...
	public boolean insertSet(BuffSet buffSet) {
		synchronized (CACHED_BUFF_SETS) {
			...
			try {
				...
				LOGGER.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "CommunityBufferDAO:insertSet(BuffSet)" );
				return false;
			} finally {
	...
	public boolean insertSkillToSet(BuffSet buffSet, int skillId) {
		...
		try {
			...
			LOGGER.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "insertSkillToSet:insertSkillToSet(BuffSet,int)" );
			return false;
		} finally {
			DbUtils.closeQuietly(con, statement);
		}
	...
	public void deleteSet(int ownerId, int setId) {
		synchronized (CACHED_BUFF_SETS) {
			...
			try {
				...
				LOGGER.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "CharacterPostFriendDAO.delete(int,int): %s", e );
				return;
			} finally {
				DbUtils.closeQuietly(con, statement);
			}
	...
	public void deleteSkillFromSet(BuffSet buffSet, int skillId) {
		...
		try {
			...
			LOGGER.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "CharacterPostFriendDAO.delete(BuffSet,int): %s", e );
			return;
		} finally {
			DbUtils.closeQuietly(con, statement);
		}
	...
	private void cleanUP() {
		...
		try {
			...
			LOGGER.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "CharacterPostFriendDAO.cleanUP(): %s", e );
			return;
		} finally {
			DbUtils.closeQuietly(con, statement);
		}
