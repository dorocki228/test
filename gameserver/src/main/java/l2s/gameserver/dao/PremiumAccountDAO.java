...
public class PremiumAccountDAO
{
	...
	private static final PremiumAccountDAO _instance;
	...
	public int[] select(String account)
	{
		...
		try
		{
			...
			statement = con.prepareStatement("SELECT type, expire_time FROM premium_accounts WHERE account=?");
	...
	public void delete(String account)
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("DELETE FROM premium_accounts WHERE account=?");
			statement.setString(1, account);
			statement.execute();
		}
		catch(Exception e)
	...
	public void insert(String account, int bonus, int endTime)
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("REPLACE INTO premium_accounts(account, type, expire_time) VALUES (?,?,?)");
			statement.setString(1, account);
	...
	static
	{
		_log = LoggerFactory.getLogger(PremiumAccountDAO.class);
		_instance = new PremiumAccountDAO();
	}
}
