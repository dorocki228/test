	...
	public List<SpawnTemplate> restore()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				SpawnTemplate template = new SpawnTemplate("from_database", null, PeriodOfDay.NONE, count, respawn, 0);
				template.addSpawnRange(new Location(x, y, z, heading));
				template.addNpc(new SpawnNpcInfo(npcId, 0, new MultiValueSet<>()));
				result.add(template);
			}
		}
		catch(Exception e)
