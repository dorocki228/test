...
public class Cubic implements Runnable
{
	...
	private boolean doCastSkill(final CubicSkillInfo info)
	{
		...
		if ((skill.isBad() || skill.isDebuff()) && !_owner.isInCombat()) {
			return false;
		}
		...
		if(target.isDead() || (target.isDoor() && !info.isCanAttackDoor()) || !canCastSkill(target, skill) || (skill.isBad() || skill.isDebuff()) && !target.isAutoAttackable(_owner))
			return false;
		...
		{
			_owner.callSkill(aimTarget, SkillEntry.makeSkillEntry(SkillEntryType.CUBIC, skill), targets, false, false, this);

			if((skill.isBad() || skill.isDebuff()) && aimTarget.isNpc())
			{
