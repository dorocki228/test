...
public class FriendList
{
	...
	private TIntObjectMap<Friend> _friendList;
	private final Player _owner;

	public FriendList(Player owner)
	{
		_friendList = new TIntObjectHashMap<>(0);
		_owner = owner;
	}
	...
	public void remove(String name)
	{
		...
		if(objectId > 0)
		{
			Player friendChar = GameObjectsStorage.getPlayer(objectId);
			_owner.sendPacket(new SystemMessage(133).addString(name), new FriendRemove(name));
			if(friendChar != null)
				friendChar.sendPacket(new SystemMessage(481).addString(_owner.getName()), new L2FriendListPacket(friendChar));
		}
		else
	...
	public IBroadcastPacket requestFriendInvite(GameObject target)
	{
		...
		new Request(Request.L2RequestType.FRIEND, _owner, player).setTimeout(10000L);
