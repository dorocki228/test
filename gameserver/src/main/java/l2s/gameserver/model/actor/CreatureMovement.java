...
public class CreatureMovement
{
	...
	{
		return (int) (((_actor.isPlayer() || _actor.isServitor()) && !_actor.isVisualTransformed() ? 16000 : 32000) / Math.max(_actor.getMoveSpeed(), 1.0));
	}
	...
	{
		// TODO remove
		if (_actor.isSummon() && target.isPlayer() && _actor.getPlayer().equals(target)) {
			return moveToLocation(loc, offset, true, false, false);
		}
	...
	public boolean updatePosition()
	{
		...
		try
		{
			...
			double speed = _actor.getMoveSpeed();
			if(speed <= 0)
			{
				stopMove();
				return false;
			...
			_moveTaskDoneDist += (now - _startMoveTime) * _previousSpeed / 1000.0;
	...
	public boolean fastPositionUpdate() {
		long now = System.nanoTime();

		final long delta = TimeUnit.NANOSECONDS.toMillis(now - _startMoveTime) / 1000;
		_moveTaskDoneDist += delta * _previousSpeed;
	 ...
	 * @param stop - рассылать ли StopMove
	 */
	public void stopMove(boolean stop, boolean updatePosition)
	{
		if(!isMoving())
			return;
