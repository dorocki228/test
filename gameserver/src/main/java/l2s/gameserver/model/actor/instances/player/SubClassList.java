...
public class SubClassList implements Iterable<SubClass>
{
	...
	public boolean restore()
	{
		...
		{
			_log.atWarning().log( "SubClassList:restore: Could not restore any sub-classes! Player: %s(%s)", _owner.getName(), _owner.getObjectId() );
			return false;
		}
		...
		for(SubClass sub : subclasses)
		{
			...
			{
				_log.atWarning().log( "SubClassList:restore: Limit is subclass! Player: %s(%s)", _owner.getName(), _owner.getObjectId() );
				break;
			}
			...
			if(_listByIndex.containsKey(sub.getIndex()))
				_log.atWarning().log( "SubClassList:restore: Duplicate index in player subclasses! Player: %s(%s)", _owner.getName(), _owner.getObjectId() );
			_listByIndex.put(sub.getIndex(), sub);

			if(_listByClassId.containsKey(sub.getClassId()))
				_log.atWarning().log( "SubClassList:restore: Duplicate class_id in player subclasses! Player: %s(%s)", _owner.getName(), _owner.getObjectId() );
			_listByClassId.put(sub.getClassId(), sub);
		}

		if(_baseSubClass == null)
		{
			_log.atWarning().log( "SubClassList:restore: Could not restore base sub-class! Player: %s(%s)", _owner.getName(), _owner.getObjectId() );
			return false;
		}
		...
		if(_activeSubClass == null)
		{
			...
			_log.atWarning().log( "SubClassList:restore: Could not restore active sub-class! Base class applied to active sub-class. Player: %s(%s)", _owner.getName(), _owner.getObjectId() );
		}

		if(_listByIndex.size() != _listByClassId.size()) // Невозможно, но хай будет.
			_log.atWarning().log( "SubClassList:restore: The size of the lists do not match! Player: %s(%s)", _owner.getName(), _owner.getObjectId() );
