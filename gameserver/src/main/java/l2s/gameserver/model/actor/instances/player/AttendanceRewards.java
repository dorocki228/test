...
{
	private static final SchedulingPattern VIP_ATTENDANCE_DATE_PATTERN;
	...
	private int _receivedRewardIndex;
	private int _nextRewardIndex;
	private ScheduledFuture<?> _loginDelayTask;
	private ScheduledFuture<?> _getNextRewardIndexTask;
	...
	public void restore()
	{
		...
		try
		{
			...
			if(Config.VIP_ATTENDANCE_REWARDS_REWARD_BY_ACCOUNT)
				receiveTime = Integer.parseInt(AccountVariablesDAO.getInstance().select(_owner.getAccountName(), "@vip_attendance_date_index", "0"));
			else
				receiveTime = _owner.getVarInt("@vip_attendance_date_index", 0);
			int index = 0;
			if(Config.VIP_ATTENDANCE_REWARDS_REWARD_BY_ACCOUNT)
				index = Integer.parseInt(AccountVariablesDAO.getInstance().select(_owner.getAccountName(), "@vip_attendance_reward_index", "0"));
			else
				index = _owner.getVarInt("@vip_attendance_reward_index", 0);
	...
	public boolean receiveReward()
	{
		...
		try
		{
			...
			if(Config.VIP_ATTENDANCE_REWARDS_REWARD_BY_ACCOUNT)
			{
				AccountVariablesDAO.getInstance().insert(_owner.getAccountName(), "@vip_attendance_reward_index", String.valueOf(getReceivedRewardIndex()));
				AccountVariablesDAO.getInstance().insert(_owner.getAccountName(), "@vip_attendance_date_index", String.valueOf((int) (System.currentTimeMillis() / 1000L)));
			}
			else
			{
				_owner.setVar("@vip_attendance_reward_index", String.valueOf(getReceivedRewardIndex()));
				_owner.setVar("@vip_attendance_date_index", String.valueOf((int) (System.currentTimeMillis() / 1000L)));
			}
			if(!_owner.hasPremiumAccount())
				_owner.sendPacket(new SystemMessagePacket(SystemMsg.YOUVE_RECEIVED_YOUR_ATTENDANCE_REWARD_FOR_DAY_S1_).addNumber(getReceivedRewardIndex()));
			else
				_owner.sendPacket(new SystemMessagePacket(SystemMsg.YOUVE_RECEIVED_YOUR_PC_CAF_ATTENDANCE_REWARD_FOR_DAY_S1_).addNumber(getReceivedRewardIndex()));
	...
	public void onEnterWorld()
	{
		if(isAvailable() && !isReceived())
		{
			_owner.sendPacket(new ExVipAttendanceItemList(_owner));
			if(!_owner.hasPremiumAccount())
				_owner.sendPacket(new SystemMessagePacket(SystemMsg.YOUR_DAY_S1_ATTENDANCE_REWARD_IS_READY_CLICK_ON_THE_REWARDS_ICON).addNumber(getNextRewardIndex()));
			else
				_owner.sendPacket(new SystemMessagePacket(SystemMsg.YOUR_DAY_S1_PC_CAF_ATTENDANCE_REWARD_IS_READY_CLICK_ON_THE_REWARDS_ICON).addNumber(getNextRewardIndex()));
		}
	}
	...
	static
	{
		VIP_ATTENDANCE_DATE_PATTERN = new SchedulingPattern("30 6 * * *");
	}
}
