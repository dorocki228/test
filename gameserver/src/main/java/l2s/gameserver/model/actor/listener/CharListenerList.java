...
public class CharListenerList extends ListenerList<Creature>
{
	...
	public void onCurrentHpDamage(double damage, Creature attacker, Skill skill, boolean sharedDamage)
	{
		if(!global.getListeners().isEmpty())
			for(Listener<Creature> listener : global.getListeners())
				if(OnCurrentHpDamageListener.class.isInstance(listener))
					((OnCurrentHpDamageListener) listener).onCurrentHpDamage(getActor(), damage, attacker, skill, sharedDamage);
		if(!getListeners().isEmpty())
			for(Listener<Creature> listener : getListeners())
				if(OnCurrentHpDamageListener.class.isInstance(listener))
					((OnCurrentHpDamageListener) listener).onCurrentHpDamage(getActor(), damage, attacker, skill, sharedDamage);
	...
	public void onChangeLocation(Location loc, int reflectionId)
	{
		if(!global.getListeners().isEmpty())
			for(Listener<Creature> listener : global.getListeners())
				if(OnChangeLocationListener.class.isInstance(listener))
					((OnChangeLocationListener) listener).location(getActor(), loc, reflectionId);
		if(!getListeners().isEmpty())
			for(Listener<Creature> listener : getListeners())
				if(OnChangeLocationListener.class.isInstance(listener))
					((OnChangeLocationListener) listener).location(getActor(), loc, reflectionId);
