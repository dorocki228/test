...
public class Mount
{
	...
	public void onRide()
	{
		switch(getType())
		{
			case WYVERN:
			{
				...
				_rider.addSkill(SkillHolder.getInstance().getSkillEntry(4289, 1), false);
				_rider.sendSkillList();
				break;
			}
		}
		_rider.unEquipWeapon();
		_rider.broadcastUserInfo(true);
	...
	public void onUnride()
	{
		...
		if(_rider.removeSkillById(325) != null)
			sendSkillList = true;
		if(_rider.removeSkillById(4289) != null)
			sendSkillList = true;
		if(sendSkillList)
			_rider.sendSkillList();
		_rider.getAbnormalList().stopEffects(4258);
		_rider.broadcastUserInfo(true);
		_rider.broadcastPacket(new RidePacket(_rider));
		_rider.broadcastUserInfo(true);
	...
	{
		return _controlItemObjId != 0 && _currentFeed < (int) (_data.getMaxMeal() * 0.01 * _data.getHungryLimit());
	}
	...
	static
	{
		_log = LoggerFactory.getLogger(Mount.class);
	}
