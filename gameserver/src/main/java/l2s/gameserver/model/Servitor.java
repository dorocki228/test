...
public abstract class Servitor extends Playable
{
	...
	protected void onSpawn()
	{
		...
		if(owner.isGMInvisible())
			startAbnormalEffect(AbnormalVisualEffect.STEALTH);
	...
	public double transferDamage(Creature attacker, double damage) {
		...
		Party party = owner.getParty();
		if (party != null) {
			if (caster != null && isInRangeZ(caster, 1000) && !caster.isDead() && party.containsMember(caster)) {
				double transferDmg = ((int) damage * (int) getStat().getValue(DoubleStat.TRANSFER_DAMAGE_TO_PLAYER, 0)) / 100.0;
				transferDmg = Math.min((int) caster.getCurrentHp() - 1, transferDmg);
				if (transferDmg > 0) {
					int membersInRange = 0;
					for (Player member : caster.getParty()) {
		...
		} else if (caster != null && caster == owner && isInRangeZ(caster, 1000) && !caster.isDead()) {
	...
	public double getChargedSoulshotPower()
	{
		if(_chargedSoulshotPower > 0)
			return getStat().getValue(DoubleStat.SOULSHOT_POWER, _chargedSoulshotPower);
		return 0.;
	}
	...
	public double getChargedSpiritshotPower()
	{
		if(_chargedSpiritshotPower > 0)
			return getStat().getValue(DoubleStat.SPIRITSHOT_POWER, _chargedSpiritshotPower);
		return 0.;
	}
	...
	public void transferOwnerBuffs()
	{
		...
		for(Abnormal a : abnormals)
		{
			...
			Abnormal abnormal = new Abnormal(a.getEffector(), this, a, false);
			abnormal.setDuration(a.getDuration());
			abnormal.setTimeLeft(a.getTimeLeft());
			getAbnormalList().add(abnormal, false);
		}
