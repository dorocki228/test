...
public class World
{
	...
	{
		_log.atInfo().log( "World: Creating regions: [%s][%s][%s].", (REGIONS_X + 1), (REGIONS_Y + 1), (REGIONS_Z + 1) );
	}
	...
	public static Stream<Creature> getAroundCharactersStream(GameObject object, int radius, int height)
	{
		int oid = object.getObjectId();
		int rid = object.getReflectionId();
		if(radius == -1)
		{
			Stream.Builder<Creature> builder = Stream.builder();
			for(GameObject o : GameObjectsStorage.getObjects())
			{
				if(!o.isCreature() || o.getObjectId() == oid || o.getReflectionId() != rid)
					continue;

				builder.add((Creature) o);
			}
			...
			for(int y = validY(currentRegion.getY() - 1); y <= validY(currentRegion.getY() + 1); y++)
				for(int z = validZ(currentRegion.getZ() - 1); z <= validZ(currentRegion.getZ() + 1); z++)
					for(GameObject obj : getRegion(x, y, z))
					{
						...
						builder.add((Creature) obj);
					}

		return builder.build();
	}

	public static Stream<Creature> getAroundCharacters(GameObject object, int radius, int height, Predicate<GameObject> predicate)
	{
		int oid = object.getObjectId();
		int rid = object.getReflectionId();
		if(radius == -1)
	...
	public static List<Playable> getAroundPlayables(GameObject object, int radius, int height)
	{
		...
		final int x0 = validX(currentRegion.getX() - numOfRegions);
	...
	public static Stream<Playable> getAroundPlayablesStream(GameObject object, int radius, int height)
	{
		int oid = object.getObjectId();
		int rid = object.getReflectionId();
		if(radius == -1)
		{
			Stream.Builder<Playable> playables = Stream.builder();
			for(GameObject o : GameObjectsStorage.getObjects())
			{
				if(!o.isPlayable() || o.getObjectId() == oid || o.getReflectionId() != rid)
					continue;
		...
		Stream.Builder<Playable> result = Stream.builder();
	...
	public static List<Player> getAroundObservers(Location loc)
	{
		...
		final List<Player> result = new ArrayList<Player>(64);
	...
	static void addZone(Zone zone)
	{
		...
		if(territory == null)
		{
			_log.atInfo().log( "World: zone - %s not has territory.", lazy(() -> zone.getName()) );
			return;
		}
