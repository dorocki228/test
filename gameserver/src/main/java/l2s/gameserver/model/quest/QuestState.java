...
public final class QuestState
{
	...
	public int getInt(String var)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s: variable %s isn\'t an integer: %s", getPlayer().getName(), var, varint );
		}
		return varint;
	}
	...
	public void giveItems(int itemId, long count, AttributeType attributeType, int power)
	{
		Player player = getPlayer();
		if(player == null)
			return;
		...
		for(int i = 0; i < count; i++)
		{
			...
			if(attributeType != AttributeType.NONE)
				item.setAttributeElement(attributeType, power);
	...
	public void killNpcByObjectId(int _objId)
	{
		...
		if(npc != null)
			...
			_log.atWarning().log( "Attemp to kill object that is not npc in quest %s", getQuest().getId() );
	}
	...
	public void showQuestionMark(boolean quest, int tutorialId)
	{
		Player player = getPlayer();
		if(player != null)
			player.sendPacket(new TutorialShowQuestionMark(quest, tutorialId));
	}
	...
	public void setCond(int cond, boolean store, String... sound)
	{
		if(cond < -1)
		{
			_log.atWarning().log( "Cannot set negate cond in quest ID[%s]!", getQuest().getId() );
			return;
		}
		...
		if(player != null)
		{
			if(getQuest().isVisible(player))
			{
				if(isStarted())
				{
					player.sendPacket(new ExShowQuestMark(getQuest().getId(), _cond.intValue()));
					if(sound.length > 0)
						playSound(sound[0]);
					else
