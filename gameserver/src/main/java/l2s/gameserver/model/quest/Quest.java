...
public class Quest implements OnInitScriptListener
{
	...
	public void addQuestItem(int... ids)
	{
		for(int id : ids)
			if(id != 0)
			{
				...
				if(i == null)
				{
					_log.atWarning().log( "Item ID[%s] is null in quest drop in %s", id, getName() );
					continue;
				}
	...
	public static void updateQuestVarInDb(QuestState qs, String var, String value)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "could not insert char quest:" );
		}
		finally
		{
	...
	public static void deleteQuestInDb(QuestState qs)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "could not delete char quest:" );
		}
		finally
		{
			DbUtils.closeQuietly(con, statement);
					...
					if(q == null)
					{
						if(!Config.DONTLOADQUEST)
						{
							if(!questsToDelete.contains(questId))
							{
								...
								_log.atWarning().log( "Unknown quest %s for player %s", questId, player.getName() );
							}
						}
						continue;
					}
	...
	public static NpcInstance addSpawnToInstance(int npcId, Location loc, int randomOffset, int refId)
	{
		...
		{
			_log.atWarning().log( "Could not spawn Npc %s", npcId );
		}
		return null;
	}
