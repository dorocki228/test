...
public class AggroList
{
	...
	private static class CommandChannelDamage extends DamageHate
	{
		private final CommandChannel commandChannel;

		CommandChannelDamage(CommandChannel commandChannel)
		{
	...
	{
		private static Comparator<DamageHate> instance = new DamageComparator().reversed();
	...
	{
		private static Comparator<DamageHate> instance = new HateComparator().reversed();
		...
		public int compare(DamageHate o1, DamageHate o2)
		{
			...
			if(o1.hate == o2.hate)
				return Integer.compare(o1.damage, o2.damage);
			return Integer.compare(o1.hate, o2.hate);
		}
	}
	...
	private final Map<CommandChannel, CommandChannelDamage> commandChannelDamageMap = new HashMap<CommandChannel, CommandChannelDamage>();
		...
		try
		{
			...
			if(attacker.getPlayer() != null)
			{
				...
				if(party != null)
				{
					...
					{
						CommandChannelDamage commandChannelDamage = commandChannelDamageMap.get(commandChannel);
						if(commandChannelDamage == null)
						{
							commandChannelDamage = new CommandChannelDamage(commandChannel);
							commandChannelDamageMap.put(commandChannel, commandChannelDamage);
						}
						commandChannelDamage.damage += damage;
					}
				}
			}
	...
	public Creature getRandomHated(int radius)
	{
		...
		List<Creature> randomHated = Arrays.stream(hated)
				.filter(ai -> ai.hate != 0)
				.map(ai -> getOrRemoveHated(ai.attackerId))
				.filter(Objects::nonNull)
				.filter(cha -> radius == -1 || cha.isInRangeZ(_npc.getLoc(), radius))
	...
	public Party getPartyWithTopDamage()
	{
		PartyDamage[] partyDamages;
		...
		try
		{
			...
			partyDamages = _partyDamageMap.values().toArray(new PartyDamage[0]);
		}
		finally
		{
			readLock.unlock();
		...
		Arrays.sort(partyDamages, DamageComparator.getInstance());

		if (partyDamages.length > 0)
			return partyDamages[0].party;
		else
			return null;
	}

	public CommandChannel getCommandChannelWithTopDamage()
	{
		CommandChannelDamage[] commandChannelDamages;

		readLock.lock();
		try
		{
			if(commandChannelDamageMap.isEmpty())
				return null;

			commandChannelDamages = commandChannelDamageMap.values().toArray(new CommandChannelDamage[0]);
		}
		finally
		{
			readLock.unlock();
		...
		Arrays.sort(commandChannelDamages, DamageComparator.getInstance());

		if (commandChannelDamages.length > 0)
			return commandChannelDamages[0].commandChannel;
		else
			return null;
	}
	...
	public Collection<CommandChannelDamage> getCommandChannelDamages()
	{
		...
		try
		{
			damages = commandChannelDamageMap.values();
		}
		finally
		{
			readLock.unlock();
	...
	public void copy(AggroList aggroList)
	{
		...
		try
		{
			...
			Collection<CommandChannelDamage> commandChannelDamages = aggroList.getCommandChannelDamages();
			for(CommandChannelDamage commandChannelDamage : aggroList.getCommandChannelDamages())
				this.commandChannelDamageMap.put(commandChannelDamage.commandChannel, commandChannelDamage);
		}
		finally
		{
			writeLock.unlock();
