...
public class RewardList extends ArrayList<RewardGroup>
{
	...
	public static RewardList parseRewardList(FluentLogger logger, Element element, RewardType type, String debugString)
	{
		...
		for(Iterator<Element> nextIterator = element.elementIterator(); nextIterator.hasNext();)
		{
			...
			if(nextName.equalsIgnoreCase("group"))
			{
				...
				for(Iterator<Element> rewardIterator = nextElement.elementIterator(); rewardIterator.hasNext();)
				{
					...
					{
						if(notGroupType)
							logger.atWarning().log( "Can\'t load rewardlist from group: %s; type: %s", debugString, type );
						else
							group.addData(data);
					}
				}
			...
			else if(nextName.equalsIgnoreCase("reward"))
			{
				if(!notGroupType)
				{
					logger.atWarning().log( "Reward can\'t be without group(and not grouped): %s; type: %s", debugString, type );
					continue;
				}
