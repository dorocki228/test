	...
	public List<RewardItem> roll(RewardType type, Player player, double penaltyMod, NpcInstance npc)
	{
		...
		switch(type)
		{
			...
			case EVENT_GROUPED:
				...
				return Collections.emptyList();
			case SWEEP:
				return rollItems(penaltyMod * player.getSpoilChanceMod(), player.getRateSpoil(), player.getSpoilCountMod());
			case RATED_GROUPED:
				if(isAdena())
					return rollAdena(penaltyMod, player.getRateAdena());

				return rollItems(penaltyMod * npc.getDropChanceMod(player), (npc != null ? npc.getRewardRate(player) : player.getRateItems()), npc.getDropCountMod(player));
			default:
				return Collections.emptyList();
		}
	}
