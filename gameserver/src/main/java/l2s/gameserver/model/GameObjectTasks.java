...
public class GameObjectTasks
{
	...
	public static class HitTask implements Runnable
	{
		...
		public HitTask(Creature cha, Creature target, int damage, boolean crit, boolean miss, boolean soulshot,
                       byte shld, boolean unchargeSS, boolean notify, int attackTime, int hitTime,
                       int elementalDamage, boolean elementalCrit)
		{
			_charRef = cha.getRef();
		...
		public void run()
		{
			...
			if(_notify)
				ThreadPoolManager.getInstance().schedule(new NotifyAITask(character, CtrlEvent.EVT_READY_TO_ACT),
						attackTime - hitTime);
		}
	}
	 ...
	 * @author UnAfraid
	 */
	public static class ResetChargesTask implements Runnable
	{
