...
public final class ItemInstance extends GameObject implements JdbcEntity
{
	...
	{
		Func[] result = Func.Companion.getEMPTY_FUNC_ARRAY();
		...
		if(lastAttacker != null) // lastAttacker в данном случае top damager
		{
			...
			if(fromNpc != null && fromNpc.isBoss()) // На эпиках, дроп поднимает лидер CC.
			{
				...
				if(group instanceof CommandChannel)
					group = group.getGroupLeader().getParty();
			}
	 ...
	 */
	private int getDefence(AttributeType attributeType)
	{
		return isArmor() ? getAttributeElementValue(attributeType, true) : 0;
	}
	...
	{
		return getDefence(AttributeType.FIRE);
	}
	...
	{
		return getDefence(AttributeType.WATER);
	}
	...
	{
		return getDefence(AttributeType.WIND);
	}
	...
	{
		return getDefence(AttributeType.EARTH);
	}
	...
	{
		return getDefence(AttributeType.HOLY);
	}
	...
	{
		return getDefence(AttributeType.UNHOLY);
	}
	 ...
	 * @return
	 */
	public int getAttributeElementValue(AttributeType attributeType, boolean withBase)
	{
		return attrs.getValue(attributeType) + (withBase ? template.getBaseAttributeValue(attributeType) : 0);
	}

	/**
	 * Возвращает элемент атрибуции предмета.<br>
	 */
	public AttributeType getAttributeElement()
	{
	...
	{
		AttributeType attributeType = isWeapon() ? getAttributeElement() : AttributeType.NONE;
		if(attributeType == AttributeType.NONE)
			return Arrays.stream(AttributeType.VALUES)
					.filter(e -> template.getBaseAttributeValue(e) > 0)
					.findFirst()
					.orElse(attributeType);
	 ...
	 * AttributeType (0 - Fire, 1 - Water, 2 - Wind, 3 - Earth, 4 - Holy, 5 - Dark, -1 - None)
	 * @param attributeType элемент
	 * @param value
	 */
	public void setAttributeElement(AttributeType attributeType, int value)
	{
		attrs.setValue(attributeType, value);
	}
	...
	public void onEquip(int slot, Playable actor)
	{
		...
		try
		{
			...
			if((flags & Inventory.UPDATE_STATS_FLAG) == Inventory.UPDATE_STATS_FLAG) {
				actor.getStat().recalculateStats(true);
			}
