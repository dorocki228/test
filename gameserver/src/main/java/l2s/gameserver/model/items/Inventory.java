...
public abstract class Inventory extends ItemContainer
{
	...
	public ItemInstance getPaperdollItem(InventorySlot slot)
	{
		return _paperdoll[slot.getSlot()];
	}
	...
	{
		return _paperdoll[slot.getSlot()] == null;
	}
	...
	private void unEquipItemInBodySlot(long bodySlot, ItemInstance item)
	{
		...
		{
			_log.atWarning().log( "Requested invalid body slot: %s, Item: %s, ownerId: \'%s\'", bodySlot, item, getOwnerId() );
			return;
		}
	...
	public void equipItem(ItemInstance item)
	{
		...
		else if(bodySlot == ItemTemplate.SLOT_L_EAR || bodySlot == ItemTemplate.SLOT_R_EAR || bodySlot == (ItemTemplate.SLOT_L_EAR | ItemTemplate.SLOT_R_EAR))
		{
			...
			else
			{
				...
				{
					if(func._stat == DoubleStat.MAGICAL_DEFENCE)
					{
		...
		{
			_log.atWarning().log( "unknown body slot:%s for item id: %s", bodySlot, item.getItemId() );
			return;
		}
	...
	public static int[] getPaperdollIndexes(long slot)
	{
		...
		if(slot == ItemTemplate.SLOT_ARTIFACT)
			return new int[]{ PAPERDOLL_ARTIFACT_1_1 };
		return new int[]{ -1 };
	}
