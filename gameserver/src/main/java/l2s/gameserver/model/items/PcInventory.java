...
public class PcInventory extends Inventory
{
	...
	{
		return _lockType != LockType.NONE && _lockItems != null && _lockItems.length > 0;
	}
	...
	public void sendAddItem(ItemInstance item)
	{
		...
		if(item.getTemplate().getAgathionMaxEnergy() > 0)
			actor.sendPacket(new ExBR_AgathionEnergyInfoPacket(1, List.of(new ItemInfo(actor, item))));
	}
