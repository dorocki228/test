...
public abstract class Playable extends Creature
{
	...
	public boolean checkPvP(final Creature target, SkillEntry skillEntry)
	{
		...
		if(skillEntry != null)
		{
			if(skillEntry.isAltUse())
				return false;
			if(skillEntry.getTemplate().getTargetTypeNew() == TargetType.DOOR_TREASURE)
				return false;
		...
		if(skillEntry == null || skillEntry.getTemplate().isBad())
		{
			if(target.isPK())
				return false;
		...
		else if(target.getPvpFlag() > 0 || target.isPK() || (target.isMonster() && !target.isDead() && skillEntry.getTemplate().isGood()))
			return true;
	...
	public void doAttack(Creature target)
	{
		...
		if(weaponItem != null)
		{
			...
			if(isBowOrCrossbow)
			{
				if(getStat().has(BooleanStat.CHEAPSHOT))
					weaponMpConsume = 0;
			}
	...
	public boolean doCast(final SkillEntry skillEntry, final Creature target, boolean forceUse)
	{
		...
		if(skill.isBad() && target.isNpc() && target.isInvulnerable() && !target.isMonster())
		{
	...
	public void callSkill(Creature aimingTarget, SkillEntry skillEntry, List<Creature> targets, boolean useActionSkills, boolean trigger, Cubic cubic)
	{
		...
		List<NpcInstance> affectedNpcs = null;

		for(Creature target : targets)
		{
			if(target.isNpc())
			{
				if(!trigger && (skill.isBad() || skill.isDebuff())) // На оффе триггеры не накладывают паралич. Проверено 20.08.16
				{
			...
			if(target.isPlayable() && player != target && !player.isMyServitor(target.getObjectId()))
			{
				...
				List<NpcInstance> npcs = World.getAroundNpc(target, 1000, 1000);
				if (affectedNpcs == null)
					affectedNpcs = new ArrayList<>(20);

				for(NpcInstance npc : npcs)
				{
					// add aggro to npc once
					...
					if (trigger || !useActionSkills || skillEntry.isAltUse() || npc.isDead()) {
						continue;
					}
		...
		super.callSkill(aimingTarget, skillEntry, targets, useActionSkills, trigger, cubic);
	}
	 ...
	 */
	public boolean isSilentMoving(double defaultChance)
	{
		double value = getStat().getValue(DoubleStat.AVOID_AGGRO, 0.0);
		if (value == 0.0) {
			value = defaultChance;
		}
