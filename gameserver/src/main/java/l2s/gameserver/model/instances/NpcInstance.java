...
import l2s.gameserver.handler.effects.impl.instant.retail.i_confuse;
...
public class NpcInstance extends Creature
{
	...
	protected void onReduceCurrentHp(double damage, Creature attacker, Skill skill, boolean awake, boolean standUp, boolean directHp, boolean isDot)
	{
		...
		if(attacker.isConfused())
		{
			for(Abnormal abnormal : attacker.getAbnormalList())
			{
				...
				{
					if(effect.getName().equals(i_confuse.class.getSimpleName()))
					{
	...
	public boolean isRaidMinion()
	{
		...
		return leader != null && leader.isRaid();
	}
	...
	public List<ItemInstance> dropItem(Player lastAttacker, int itemId, long itemCount)
	{
		if(itemCount == 0 || lastAttacker == null)
			return Collections.emptyList();

		List<ItemInstance> result = new ArrayList<>((int) itemCount);
	...
	public boolean canAttackAnotherNpc()
	{
		return false;
	}
	...
	{
		return getStat().getValue(DoubleStat.BONUS_EXP, getTemplate().rewardExp);
	}

	public double getSpReward()
	{
		return getStat().getValue(DoubleStat.BONUS_SP, getTemplate().rewardSp);
	}
	...
	public void broadcastCharInfoImpl(IUpdateTypeComponent... components)
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).log( "%s: Trying broadcast char info without components!", getClass().getSimpleName() );
			return;
		}
	...
	public void showQuestWindow(Player player, int questId)
	{
		...
		{
			_log.atWarning().log( "problem with npc text(QUEST ID[%s]%s", questId, e );
	...
	{
		if(npc == null || player.isDead() || player.isInTrade() || player.isSitting() || player.getSittingTask())
		{
			player.sendActionFailed();
			return false;
		}
		...
		try
		{
			...
			else if(command.startsWith("Quest"))
			{
				...
				else
				{
					...
					{
						_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(nfe).log( "" );
					}
				}
			}
			...
			else if(cmd.equalsIgnoreCase("ensoul"))
			{
				...
				if(cmd2.equalsIgnoreCase("add"))
					...
					player.sendPacket(ExShowEnsoulExtractionWindow.STATIC);
			}
			...
			else
			{
				...
				if(b != null)
					...
					_log.atWarning().log( "Unknown command=[%s] npcId:%s", command, getTemplate().getId() );
			}
		}
		catch(NumberFormatException nfe)
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(nfe).log( "Invalid bypass to Server command parameter! npcId=%s command=[%s]", getTemplate().getId(), command );
		}
		catch(Exception sioobe)
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(sioobe).log( "Incorrect htm bypass! npcId=%s command=[%s]", getTemplate().getId(), command );
		}
	}
	...
	public boolean teleportPlayer(Player player, int x, int y, int z, int itemId, long itemCount, int[] castleIds, int reflectionId)
	{
		...
		if(reflectionId > -1)
		{
			...
			if(reflection == null)
			{
				_log.atWarning().log( "Cannot teleport to reflection ID: %s!", reflectionId );
				return false;
			}
	...
	public static void showCertificationSkillList(Player player)
	{
	...
	public int calculateLevelDiffForDrop(int charLevel)
	{
		...
		int levelDiff = Math.abs(charLevel - mobLevel);
		return Math.min(Math.max(levelDiff - deepblue_maxdiff, 0), 9);
	}
	...
	{
		return _parameters.getObject(str);
	}

	public Object getParameter(String str, Object defaultValue)
	{
		return _parameters.getObject(str, defaultValue);
	}
