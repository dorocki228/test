...
public class PetInstance extends Servitor
{
	...
	public static final PetInstance restore(ItemInstance control, NpcTemplate template, Player owner)
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "Could not restore Pet data from item: %s!", control );
			return null;
		}
		finally
		{
	...
	private void destroyControlItem()
	{
		...
		{
			_log.atWarning().log( "could not delete pet:%s", e );
		}
		finally
		{
	...
	public int getMaxLoad()
	{
		...
		double baseLoad = Math.floor(BaseStats.CON.calcBonus(this) * _data.getMaxLoad(_level) * Config.MAXLOAD_MODIFIER);
	...
	public void store()
	{
		...
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "Could not store pet data!" );
		}
		finally
		{
			DbUtils.closeQuietly(con, statement);
	...
	{
		// TODO should don't work like this in classic ?
		// onAttacked(attacker);
	}
