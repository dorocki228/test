...
public class SummonInstance extends Servitor
{
	...
	private final boolean _unSummonExisting;
	...
	public SummonInstance(int objectId, NpcTemplate template, Player owner, int lifetime, int consumeid, int consumecount, int consumedelay, int summonPoints, Skill skill, boolean saveable)
	{
		super(objectId, template, owner);
		_expPenalty = 0.0;
		_attackMode = AttackMode.PASSIVE;
		...
		_disappearTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new Lifetime(), 5000L, 5000L);
		...
		_unSummonExisting = template.getAIParams().getBool("unsummon_existing", false);
	}
	...
	protected void onSpawn()
	{
		...
		if(owner != null)
		{
			Iterator<SummonInstance> iterator = owner.getSummons().iterator();
			if(iterator.hasNext())
			{
				SummonInstance summon = iterator.next();
				setAttackMode(summon.getAttackMode());
			}
		}
	}
	...
	{
		ClassLevel classLevel = getPlayer().getClassLevel();
	...
	public boolean isUnSummonExisting()
	{
		return _unSummonExisting;
	}
