...
public class Clan implements Iterable<UnitMember>
{
	...
	public static SchedulingPattern CHANGE_LEADER_TIME_PATTERN;

	static {
		String clanChangeLeaderTime = Config.CLAN_CHANGE_LEADER_TIME;
		CHANGE_LEADER_TIME_PATTERN = clanChangeLeaderTime.isBlank() ? null : new SchedulingPattern(clanChangeLeaderTime);
	}
	...
	private void updateClanAttendanceInfoInDB()
	{
		if(getClanId() == 0)
		{
			_log.atWarning().log( "updateClanAttendanceInDB with empty ClanId" );
			Thread.dumpStack();
			return;
		}
		...
		{
			_log.atWarning().log( "error while updating clan attendance \'%s\' data in db", getClanId() );
	...
	private void updateClanScoreInDB()
	{
		if(getClanId() == 0)
		{
			_log.atWarning().log( "updateClanScoreInDB with empty ClanId" );
			Thread.dumpStack();
			return;
		}
		...
		{
			_log.atWarning().log( "error while updating clan reputation score \'%s\' data in db", getClanId() );
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "" );
		}
		finally
		{
	...
	public void updateClanInDB()
	{
		if(getLeaderId() == 0)
		{
			_log.atWarning().log( "updateClanInDB with empty LeaderId" );
			Thread.dumpStack();
			return;
		}
		...
		if(getClanId() == 0)
		{
			_log.atWarning().log( "updateClanInDB with empty ClanId" );
			Thread.dumpStack();
			return;
		}
		Connection con = null;
		...
		{
			_log.atWarning().log( "error while updating clan \'%s\' data in db", getClanId() );
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "" );
		}
		finally
		{
	...
	public void store()
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "Exception: %s", e );
		}
		finally
		{
			DbUtils.closeQuietly(con, statement);
	...
	public static Clan restore(int clanId)
	{
		...
		try
		{
			...
			{
				_log.atWarning().log( "Clan %s doesnt exists!", clanId );
				return null;
			}
		}
		catch(Exception e)
		{
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "Error while restoring clan!" );
		}
		finally
		{
		...
		if(clan == null)
		{
			_log.atWarning().log( "Clan %s does\'t exist", clanId );
			return null;
		}
	...
	private void restoreSkills()
	{
		...
		{
			_log.atWarning().log( "Could not restore clan skills: %s", e );
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "" );
		}
		finally
		{
	...
	public SkillEntry addSkill(SkillEntry newSkillEntry, boolean store)
	{
		...
		if(newSkillEntry != null)
		{
			...
			if(store)
			{
				...
				{
					_log.atWarning().log( "Error could not store char skills: %s", e );
					_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "" );
				}
				finally
				{
			...
			ExPledgeSkillAdd p = new ExPledgeSkillAdd(newSkillEntry.getId(), newSkillEntry.getLevel());
			ExPledgeSkillList p2 = new ExPledgeSkillList(this);
			for(UnitMember temp : this)
			{
				if(temp.isOnline())
				{
	...
	public void removeSkill(int skill, boolean store)
	{
		...
		if(store)
		{
			...
			{
				_log.atWarning().log( "Error could not delete char skills: %s", e );
				_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "" );
			}
			finally
			{
		...
		ExPledgeSkillAdd p = new ExPledgeSkillAdd(skill, 0);
		for(UnitMember temp : this)
		{
			Player player = temp.getPlayer();
	...
	public void broadcastSkillListToOnlineMembers()
	{
		for(UnitMember temp : this)
		{
			...
			if(player != null && player.isOnline())
			{
				player.sendPacket(new ExPledgeSkillList(this));
				player.sendSkillList();
			}
		}
	}
	...
	public final void addSubUnit(SubUnit sp, boolean updateDb)
	{
		...
		if(updateDb)
		{
			broadcastToOnlineMembers(new ExSubPledgeUpdated(sp));
			Connection con = null;
			PreparedStatement statement = null;
			try
			{
			...
			{
				_log.atWarning().log( "Could not store clan Sub pledges: %s", e );
				_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "" );
			}
			finally
			{
	...
	private void restoreSubPledges()
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "Could not restore clan SubPledges: %s", e );
		}
		finally
		{
			DbUtils.closeQuietly(con, statement, rset);
	...
	public int getSubPledgeLimit(int pledgeType)
	{
		...
		switch(pledgeType)
		{
			case SUBUNIT_MAIN_CLAN:
				...
				{
					case 0:
						limit = Config.CLAN_LEVEL_1_LIMIT;
						break;
					case 1:
						limit = Config.CLAN_LEVEL_2_LIMIT;
						break;
					case 2:
						limit = Config.CLAN_LEVEL_3_LIMIT;
						break;
					case 3:
						limit = Config.CLAN_LEVEL_4_LIMIT;
						break;
					default:
						limit = Config.CLAN_LEVEL_5_LIMIT;
						break;
				}
				break;
			case SUBUNIT_ACADEMY:
		...
		try
		{
			...
			while(rset.next())
			{
				...
				if(p != null)
					...
					_log.atWarning().log( "Invalid rank value (%s), please check clan_privs table", rank );
			}
		}
		catch(Exception e)
		{
			_log.atWarning().log( "Could not restore clan privs by rank: %s", e );
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "" );
		}
		finally
		{
	...
	public RankPrivs getRankPrivs(int rank)
	{
		...
		{
			_log.atWarning().log( "Requested invalid rank value: %s", rank );
			Thread.dumpStack();
			return null;
		}
		if(_privs.get(rank) == null)
		{
			_log.atWarning().log( "Request of rank before init: %s", rank );
			Thread.dumpStack();
	...
	public void setRankPrivs(int rank, int privs)
	{
		if(rank < RANK_FIRST || rank > RANK_LAST)
		{
			_log.atWarning().log( "Requested set of invalid rank value: %s", rank );
			Thread.dumpStack();
			return;
		}
		...
		{
			_log.atWarning().log( "Could not store clan privs for rank: %s", e );
			_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "" );
		}
		finally
		{
	...
	public void refreshAttendanceInfo()
	{
		...
		broadcastToOnlineMembers(ExPledgeActivityMarkReset.STATIC); // TODO: Правильно ли посылаем данный пакет?
