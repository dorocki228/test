		...
		try
		{
			...
			if(_type != Clan.SUBUNIT_ACADEMY)
			{
				...
				if(leader != null)
					...
					_log.atSevere().log( "Clan %s have no leader!", _name );
			}
		}
		catch(Exception e)
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "Error while restoring clan members for clan: %s %s", _clan.getClanId(), e );
		}
		finally
		{
