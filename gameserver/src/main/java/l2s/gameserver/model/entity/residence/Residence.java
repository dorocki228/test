...
public abstract class Residence implements JdbcEntity
{
	...
	protected void loadFunctions()
	{
		...
		{
			_log.atWarning().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "Residence: loadFunctions(): %s", e );
		}
		finally
		{
	...
	public boolean updateFunctions(ResidenceFunctionType type, int level)
	{
		...
		{
			_log.atWarning().log( "Exception: updateFunctions(ResidenceFunctionType,int): %s", e );
		}
		finally
		{
	...
	public void removeFunction(ResidenceFunctionType type)
	{
		...
		{
			_log.atWarning().log( "Exception: removeFunction(int type): %s", e );
		}
		finally
		{
			DbUtils.closeQuietly(con, statement);
	...
	public void removeFunctions()
	{
		...
		{
			_log.atWarning().log( "Exception: removeFunctions(): %s", e );
		}
		finally
		{
			DbUtils.closeQuietly(con, statement);
