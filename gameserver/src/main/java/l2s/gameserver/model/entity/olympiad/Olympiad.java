...
public class Olympiad
{
	...
	public static final int[][] BUFFS_LIST = { { 4357, 2 }, // Haste
			{ 4355, 3 }, // Acumen
			{ 4342, 2 }, // Wind Walk
			{ 4345, 3 }, // Might
			{ 4344, 3 }, // Shield
	...
	public static void load()
	{
		...
		switch(_period)
		{
			...
			case 1:
				...
				break;
			default:
				_log.atWarning().log( "Olympiad System: Omg something went wrong in loading!! Period = %s", _period );
				return;
		}

		_log.atInfo().log( "Olympiad System: Loading Olympiad System...." );
		if(_period == 0)
			_log.atInfo().log( "Olympiad System: Currently in Olympiad Period" );
		else
			_log.atInfo().log( "Olympiad System: Currently in Validation Period" );

		_log.atInfo().log( "Olympiad System: Period Ends...." );
		...
		_log.atInfo().log( "Olympiad System: In %s days, %s hours and %s mins.", numDays, numHours, numMins );

		if(_period == 0)
		{
			_log.atInfo().log( "Olympiad System: Next Weekly Change is in...." );
			...
			_log.atInfo().log( "Olympiad System: In %s days, %s hours and %s mins.", numDays2, numHours2, numMins2 );
		}

		_log.atInfo().log( "Olympiad System: Loaded %s participants.", lazy(() -> _participants.size()) );
	...
	private static synchronized void updateCompStatus()
	{
		...
		_log.atInfo().log( "Olympiad System: Competition Period Starts in %s days, %s hours and %s mins.", numDays, numHours, numMins );
		_log.atInfo().log( "Olympiad System: Event starts/started: %s", lazy(() -> _compStart.getTime()) );
	...
	private static long setNewCompBegin()
	{
		...
		_log.atInfo().log( "Olympiad System: New Schedule @ %s", lazy(() -> _compStart.getTime()) );
