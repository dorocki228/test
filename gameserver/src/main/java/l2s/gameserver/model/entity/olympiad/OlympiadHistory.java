...
public class OlympiadHistory
{
	...
	public String toString(Player player, int target, int wins, int loss, int tie)
	{
        ...
        String main = _gameStatus == 0 ? StringsHolder.getInstance().getString(player, "hero.history.tie") : team == _gameStatus ? StringsHolder.getInstance().getString(player, "hero.history.win") : StringsHolder.getInstance().getString(player, "hero.history.loss");
