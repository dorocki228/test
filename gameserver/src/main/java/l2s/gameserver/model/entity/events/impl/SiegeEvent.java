...
public abstract class SiegeEvent<R extends Residence, S extends SiegeClanObject> extends Event
{
	...
	public void teleportPlayers(String t)
	{
		...
		for(Player player : players)
		{
			...
			if(t.equalsIgnoreCase(HAVE_OWNER) || t.equalsIgnoreCase(DEFENDERS))
				...
				loc = TeleportUtils.getRestartPoint(player, RestartType.VILLAGE).getLoc();
			else
	...
	public void checkRestartLocs(Player player, Map<RestartType, Boolean> r)
	{
		...
		if(clan != null)
			if(clan.getFlag() != null)
				r.put(RestartType.BATTLE_CAMP, Boolean.TRUE);
	}
	...
	public Location getRestartLoc(Player player, RestartType type)
	{
		...
		if (type == RestartType.BATTLE_CAMP)
		{
	...
	public List<Player> itemObtainPlayers()
	{
		...
		return getPlayersInZone().stream()
				.filter(player -> player.containsEvent(this))
