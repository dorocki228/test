...
public class CTBTeamObject implements SpawnableObject
{
	...
	public void spawnObject(Event event)
	{
		if(_flag == null)
		{
			(_flag = new NpcInstance(IdFactory.getInstance().getNextId(), _flagTemplate, StatsSet.EMPTY)).setCurrentHpMp(_flag.getMaxHp(), _flag.getMaxMp());
		...
		else
		{
			...
			NpcTemplate template = _siegeClan == null || _siegeClan.getParam() == 0L ? _mobTemplate : NpcHolder.getInstance().getTemplate((int) _siegeClan.getParam());
			(_mob = (CTBBossInstance) template.getNewInstance()).setCurrentHpMp(_mob.getMaxHp(), _mob.getMaxMp());
			_mob.setMatchTeamObject(this);
			_mob.addEvent(event);
			int x = (int) (_flagLoc.x + 300.0 * Math.cos(_mob.headingToRadians(_flag.getHeading() - 32768)));
			int y = (int) (_flagLoc.y + 300.0 * Math.sin(_mob.headingToRadians(_flag.getHeading() - 32768)));
