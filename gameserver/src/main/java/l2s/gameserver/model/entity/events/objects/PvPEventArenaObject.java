...
public class PvPEventArenaObject extends Reflection
{
	...
	public PvPEventArenaObject(PvPEvent pvpEvent, int teams)
	{
		...
		{
			_teamsList.add(ConcurrentHashMap.newKeySet());
		}
	}
...
//	}
	...
	public void teleportPlayers()
	{
		for(int i = 0; i < _teamsList.size(); i++)
		{
			for(PvPEventPlayerObject member : _teamsList.get(i))
			{
				...
				_pvpEvent.abnormals(player, true);
				addHook(player);
				player.setPreserveAbnormal(true);
		...
		for(int i = 0; i < _teamsList.size(); i++)
		{
			for(PvPEventPlayerObject member : _teamsList.get(i))
			{
				...
				if (!player.isTeleporting()) {
					...
					ThreadPoolManager.getInstance().schedule(() -> removePlayer(player), 10, TimeUnit.SECONDS);
				}
			}
		}
	...
	public void startSpawnProtection(Player player)
	{
		player.setNonPvpTime(System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(5));
	}
	...
	public void removePlayer(Player player)
	{
		...
		player.setPreserveAbnormal(false);
