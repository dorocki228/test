...
public class BoatWayEvent extends Event
{
	...
	public void startEvent()
	{
		...
		for(Player player : _boat.getPlayers())
			if(_ticketId > 0)
			{
				if(player.consumeItem(_ticketId, 1L, true))
				{
					if(startPacket == null)
						continue;
	...
	public void moveNext()
	{
		...
		BoatPoint bp = points.get(_boat.getRunState());
		if(bp.getSpeed1() >= 0)
	...
	public List<Player> broadcastPlayers(int range)
	{
		...
		if(range > 0)
			...
			for(Location loc : _broadcastPoints)
			{
				...
				{
					if(!player2.getReflection().isMain())
						continue;
					int tx = MapUtils.regionX(player2) - rx;
					int ty = MapUtils.regionY(player2) - ry;
					if(tx * tx + ty * ty > Config.SHOUT_SQUARE_OFFSET || players.contains(player2))
						continue;
					players.add(player2);
				}
			}
