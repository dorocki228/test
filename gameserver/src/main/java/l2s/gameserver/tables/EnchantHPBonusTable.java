...
public class EnchantHPBonusTable
{
	...
	private EnchantHPBonusTable()
	{
		try
		{
				...
				if("list".equalsIgnoreCase(n.getNodeName()))
					for(Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
					{
						...
						if("options".equalsIgnoreCase(d.getNodeName()))
						{
							...
							if(att == null)
							{
								_log.atInfo().log( "EnchantHPBonusTable: Missing onepiece_factor, skipping" );
								continue;
							}
						...
						else if("enchant_bonus".equalsIgnoreCase(d.getNodeName()))
						{
							...
							if(att == null)
							{
								_log.atInfo().log( "EnchantHPBonusTable: Missing grade, skipping" );
								continue;
							}
							...
							if(att == null)
							{
								_log.atInfo().log( "EnchantHPBonusTable: Missing bonus id: %s, skipping", lazy(() -> grade.ordinal()) );
								continue;
							}
							...
							for(int i = 0; i < tokenCount; i++)
							{
								...
								if(value == null)
								{
									_log.atInfo().log( "EnchantHPBonusTable: Bad Hp value!! grade: %s token: %s", lazy(() -> grade.ordinal()), i );
									value = 0;
								}
								bonus[i] = value;
							}
			...
			_log.atInfo().log( "EnchantHPBonusTable: Loaded bonuses for %s grades.", lazy(() -> _armorHPBonus.size()) );
		}
		catch(Exception e)
		{
			_log.atWarning().log( "EnchantHPBonusTable: Lists could not be initialized." );
			e.printStackTrace();
		}
	}
