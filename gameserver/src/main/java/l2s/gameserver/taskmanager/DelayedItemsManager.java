...
public class DelayedItemsManager implements Runnable
{
	...
	public static void addDelayed(int objectId, int itemId, long itemCount, int enchant, String desc)
	{
		...
		{
			_log.atInfo().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "DelayedItemsManager.addDelayed(int, int, long): %s", e );
		}
		finally
		{
	...
	public int loadDelayed(Player player, boolean notify)
	{
		...
		synchronized (_lock)
		{
					...
					if(ITEM_TEMPLATE != null)
					{
						...
						for(int i = 0; i < (stackable ? 1 : ITEM_COUNT); i++)
						{
							if(ITEM_COUNT > 0)
							{
								...
								if(newItem == null)
								{
									_log.atWarning().log( "Unable to delayed create item %s request %s", ITEM_ID, PAYMENT_ID );
									continue;
								}
								...
								ItemLogger.INSTANCE.log(ItemLogger.ItemProcess.DelayedItemReceive, player, newItem, DESCRIPTION);
							}
			...
			{
				_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "Could not load delayed items for player %s!", player );
			}
			finally
			{
