	...
	public static class GeoBlocksMatchFinder extends RunnableImpl
	{
		...
		private void saveToFile(final BlockLink[] links)
		{
			...
			try
			{
				...
				final ByteBuffer buffer = wChannel.map(FileChannel.MapMode.READ_WRITE, 0, links.length*6+1);
		...
		private BlockLink[] gen()
		{
			...
			final List <BlockLink> links = new ArrayList <BlockLink>();
	...
	public static class CheckSumLoader extends RunnableImpl
	{
		...
		private boolean loadFromFile()
		{
			...
			try
			{
				...
				final ByteBuffer buffer = roChannel.map(FileChannel.MapMode.READ_ONLY, 0, roChannel.size());
				roChannel.close();
				buffer.order(ByteOrder.LITTLE_ENDIAN);
		...
		private void saveToFile()
		{
			...
			try
			{
				...
				final ByteBuffer buffer = wChannel.map(FileChannel.MapMode.READ_WRITE, 0, GeoEngine.BLOCKS_IN_MAP*4);
				buffer.order(ByteOrder.LITTLE_ENDIAN);
