...
public class GeoOptimizer
{
	...
	public static class GeoBlocksMatchFinder implements Runnable
	{
		...
		{
			log.atInfo().log( "Saving matches to: %s", fileName );
			try
			{
			...
			{
				log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "" );
			}
		}
		...
		{
			log.atInfo().log( "Searching matches for %s_%s", rx, ry );
			...
			log.atInfo().log( "Founded %s matches for %s_%s in %ss", lazy(() -> links.size()), rx, ry, started / 1000f );
			return links.toArray(new BlockLink[links.size()]);
		}
	...
	public static class CheckSumLoader implements Runnable
	{
		...
		{
			log.atInfo().log( "Saving checksums to: %s", fileName );
		...
		private void gen()
		{
			log.atInfo().log( "Generating checksums for %s_%s", rx, ry );
			int[] _checkSums = new int[GeoEngine.BLOCKS_IN_MAP];
