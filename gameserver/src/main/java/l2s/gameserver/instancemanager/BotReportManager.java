...
public final class BotReportManager
{
	...
	private BotReportManager()
	{
		if(Config.BOTREPORT_ENABLED)
		{
			...
			_charRegistry = new CHashIntObjectMap<>();
			_reports = new CHashIntObjectMap<>();
	...
	private void loadReportedCharData()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				{
					ReporterCharData rcd2 = _charRegistry.get(reporter);
					if(rcd2 != null)
						rcd2.setPoints(rcd2.getPointsLeft() - 1);
					else
					{
						rcd2 = new ReporterCharData();
						rcd2.setPoints(6);
						_charRegistry.put(reporter, rcd2);
					}
				}
			}
	...
	public boolean reportBot(Player reporter)
	{
		...
		SystemMessagePacket sm2 = new SystemMessagePacket(SystemMsg.YOU_HAVE_USED_A_REPORT_POINT_ON_C1_YOU_HAVE_S2_POINTS_REMAINING_ON_THIS_ACCOUNT);
		sm2.addName(bot);
		sm2.addNumber(rcdRep.getPointsLeft());
		reporter.sendPacket(sm2);
