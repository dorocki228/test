...
public class PCCafeCouponManager
{
	...
	public String generateCode(int type, String value)
	{
		...
		try
		{
			for(int i = 0; i < 100; i++) // 100 попыток, избегаем зацикливания.
			{
				...
				{
					_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s: Error while generate coupon code.", getClass().getSimpleName() );
				}
				finally
				{
	...
	public boolean requestEnterCode(Player player, String couponCode)
	{
		...
		try
		{
			...
			{
				_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s: Error while reading coupon code.", getClass().getSimpleName() );
			}
			finally
			{
	...
	private boolean useCoupon(Player player, String couponCode, int type, String value)
	{
		...
		try
		{
			...
			if(handler == null)
			{
				_log.atWarning().log( "%s: Not found handler for coupon TYPE[%s]!", getClass().getSimpleName(), type );
				return false;
			}
			...
			{
				_log.atSevere().withStackTrace(com.google.common.flogger.StackSize.FULL).withCause(e).log( "%s: Error while use coupon code.", getClass().getSimpleName() );
			}
			finally
			{
				DbUtils.closeQuietly(con, statement, rset);
