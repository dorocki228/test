...
public class GameBanManager extends BanManager {
	...
	public void init() {
		...
		LOGGER.atInfo().log( "GameBanManager: Initialized." );
	}
	...
	public static void onBan(BanBindType bindType, Object bindValueObj, BanInfo banInfo, boolean auth) {
		...
		if (bindType == BanBindType.LOGIN) {
			GameClient client = AuthServerClientService.INSTANCE.getWaitingClient(bindValue);
			if (client != null)
				gameClients.add(client);

			client = AuthServerClientService.INSTANCE.getAuthedClient(bindValue);
			if (client != null)
				gameClients.add(client);
		} else if (bindType == BanBindType.IP) {
			gameClients.addAll(AuthServerClientService.INSTANCE.getWaitingClientsByIP(bindValue));
			gameClients.addAll(AuthServerClientService.INSTANCE.getAuthedClientsByIP(bindValue));
		} else if (bindType == BanBindType.HWID) {
			gameClients.addAll(AuthServerClientService.INSTANCE.getWaitingClientsByHWID(bindValue));
			gameClients.addAll(AuthServerClientService.INSTANCE.getAuthedClientsByHWID(bindValue));
		...
		for (GameClient gameClient : gameClients) {
			AuthServerClientService.INSTANCE.removeClient(gameClient);
