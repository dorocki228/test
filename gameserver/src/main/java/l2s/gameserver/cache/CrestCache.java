	...
	public void load()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				{
					_log.atWarning().log( "Clan large crest has crashed. Clan ID: %s", pledgeId );
					continue;
				}
		...
		_log.atInfo().log( "CrestCache: Loaded %s crests", count );
	}
	...
	public int savePledgeCrestLarge(int pledgeId, int crestPart, int crestTotalSize, byte[] data)
	{
		...
		try
		{
			...
			if(crest != null)
			{
				...
				{
					_pledgeCrestLargeTemp.remove(pledgeId);
					_log.atWarning().log( "Error while save pledge large crest, clan_id: %s, crest_part: %s, crest_total_size: %s, temp_size: %s", pledgeId, crestPart, crestTotalSize, tempSize );
				}
				else
				{
				...
				_log.atWarning().log( "Error while save pledge large crest, clan_id: %s, crest_part: %s, crest_total_size: %s", pledgeId, crestPart, crestTotalSize );
		}
		finally
		{
			writeLock.unlock();
