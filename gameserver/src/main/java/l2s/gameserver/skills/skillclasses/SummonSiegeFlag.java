	...
	public boolean checkCondition(Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		switch(_flagType)
		{
			...
			case ADVANCED:
			{
				...
				Castle castle = ResidenceHolder.getInstance().findNearestResidence(Castle.class, activeChar.getX(), activeChar.getY(), activeChar.getZ(), activeChar.getReflection(), 65536);
	...
	public void onEndCast(Creature activeChar, List<Creature> targets)
	{
		...
		switch(_flagType)
		{
			...
			default:
			{
				...
				int x = (int) (player.getX() + 100.0 * Math.cos(player.headingToRadians(player.getHeading() - 32768)));
				int y = (int) (player.getY() + 100.0 * Math.sin(player.headingToRadians(player.getHeading() - 32768)));
