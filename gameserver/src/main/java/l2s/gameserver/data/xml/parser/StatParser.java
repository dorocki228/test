...
public abstract class StatParser<H extends AbstractHolder> extends AbstractParser<H>
{
	...
	protected Condition parseFirstCond(Element sub, boolean parseNewConds, int... arg)
	{
	...
	protected Condition parseTargetCondition(Element element, int... arg)
	{
		...
		for(Iterator<Attribute> iterator = element.attributeIterator(); iterator.hasNext();)
		{
			...
			if(name.equalsIgnoreCase("is_pet_feed"))
				...
				cond = joinAnd(cond, new ConditionTargetDirection(Position.valueOf(value.toUpperCase())));
	...
	protected Condition parsePlayerCondition(Element element, int... arg)
	{
		...
		for(Iterator<Attribute> iterator = element.attributeIterator(); iterator.hasNext();)
		{
			...
			else if(name.equalsIgnoreCase("has_charges"))
				cond = joinAnd(cond, new ConditionPlayerHasCharges(Integer.parseInt(value)));
	...
	protected void parseFor(Element forElement, StatTemplate template, int... arg)
	{
		for(Iterator<Element> iterator = forElement.elementIterator(); iterator.hasNext();)
		{
			...
			if(elementName.equals("add"))
				...
				// FIXME Временный изжоп, до переписывания системы статтов под ПТС.
			else if(elementName.startsWith("p_"))
			{
				if(elementName.toLowerCase().startsWith("p_elemental_"))
				{
					String elementalElementName = parseString(element.attributeValue("element"), arg);
					DoubleStat stat = DoubleStat.Companion.find(elementName.replaceFirst("^p_", elementalElementName + "_"));
				...
				{
					DoubleStat stat = DoubleStat.Companion.find(elementName.replaceFirst("^p_", ""));
					attachNewFunc(element, template, stat, arg);
				}
			}
		}
	...
	protected void parseTriggers(Element f, StatTemplate triggerable, int... arg)
	{
		for(Iterator<Element> iterator = f.elementIterator(); iterator.hasNext();)
		{
			...
			Condition condition = parseFirstCond(element, false, arg);
			if(condition != null)
	...
	{
		DoubleStat stat = DoubleStat.Companion.find(n.attributeValue("stat"));
		String order = n.attributeValue("order");
		int ord = order == null ? defaultOrder : parseNumber(order, arg).intValue();
		Condition applyCond = parseFirstCond(n, false, arg);
		double val = 0;
	...
	protected void attachNewFunc(Element n, StatTemplate template, DoubleStat stat, StatModifierType type, int... arg)
	{
		Condition applyCond = parseFirstCond(n, false, arg);
	...
	protected void attachNewFunc(Element n, StatTemplate template, DoubleStat stat, int... arg)
	{
