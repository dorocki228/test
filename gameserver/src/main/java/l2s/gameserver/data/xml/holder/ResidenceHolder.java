...
{
	private static final ResidenceHolder _instance;
	private final IntObjectMap<Residence> _residences;
	...
	{
		_residences = new TreeIntObjectMap<>();
	}
	...
	{
		List<R> residences = new ArrayList<>();
	...
	{
		Collection<Residence> residences = (Collection<Residence>) (type == null ? getResidences() : getResidenceList(type));
	...
	public <R extends Residence> R findNearestResidence(Class<R> clazz, int x, int y, int z, Reflection ref, int offset)
	{
		...
		if(residence == null)
		{
			...
			for(Residence r : getResidenceList(clazz))
			{
				...
				if(zone == null)
					continue;
				double distance = zone.findDistanceToZone(x, y, z, false);
