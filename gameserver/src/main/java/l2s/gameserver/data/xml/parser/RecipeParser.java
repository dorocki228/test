...
public final class RecipeParser extends AbstractParser<RecipeHolder>
{
	...
	protected void readData(Element rootElement, boolean custom) throws Exception
	{
		for(Iterator<Element> iterator = rootElement.elementIterator(); iterator.hasNext();)
		{
			...
			final boolean isCommon = element.attributeValue("is_common") != null && Boolean.parseBoolean(element.attributeValue("is_common"));
			boolean canBeCriticalCrafted = true;
				...
				else if("products".equalsIgnoreCase(subElement.getName()))
				{
					for(Element e : subElement.elements())
					{
						if("item".equalsIgnoreCase(e.getName()))
						{
							...
							if (!template.isWeapon() && !template.isArmor() && !template.isJewelry()) {
