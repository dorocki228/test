...
public final class ItemHolder extends AbstractHolder
{
	...
	public void addItem(ItemTemplate template, boolean custom)
	{
		if (_items.containsKey(template.getItemId())) {
			if (custom)
				_log.atInfo().log("Item %d was overridden by custom.", template.getItemId());
			else
				_log.atInfo().log("Item %d was overridden.", template.getItemId());
		}
