 ...
 * @author Bonux
 */
public class NpcAI<T extends NpcInstance> extends CharacterAI<T>
{
	...
	private List<Creature> _neighbors = Collections.emptyList();
	...
	private boolean isLookingForNeighbors = false;
	...
	public void setLookingForNeighbors(boolean lookingForNeighbors) {
		isLookingForNeighbors = lookingForNeighbors;
	}
		...
		try
		{
			...
			if((now - _lookNeighborTimestamp) > 500L)
			{
				...
				if (_neighbors.isEmpty()) {
					_neighbors = actor.getAroundCharacters(range, 250, creature -> !creature.isInvisible(actor))
							.peek(creature -> notifyEvent(CtrlEvent.EVT_SEE_CREATURE, creature))
