...
public class AdminEffects implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		int val;
		AbnormalVisualEffect ae = AbnormalVisualEffect.NONE;
		...
		switch(command)
		{
			...
			case admin_vis:
				if(activeChar.isGMInvisible())
				{
					...
					{
						servitor.stopAbnormalEffect(AbnormalVisualEffect.STEALTH);
						for(Player p : players)
						{
				...
				else
				{
					...
					for(Servitor servitor : activeChar.getServitors())
					{
						servitor.startAbnormalEffect(AbnormalVisualEffect.STEALTH);
						for(Player p : players)
						{
			...
			case admin_gmspeed:
				...
				addGmSpeedStats(targetCreature, val);
		...
		switch(command)
		{
			...
			case admin_abnormal:
				try
				{
					if(wordList.length > 1)
						ae = AbnormalVisualEffect.valueOf(Integer.parseInt(wordList[1]));
				}
				catch(Exception e)
				{
				...
				if(ae == null || ae == AbnormalVisualEffect.NONE)
				{
			...
			case admin_showusm:
				...
				activeChar.sendPacket(new ExSendUsmEvent(usmId));
				break;
	...
	public static void addGmSpeedStats(Creature targetCreature, int val) {
		SPEED_STATS.forEach(speedStat -> targetCreature.getStat().removeFixedValue(speedStat));
		if (val > 0)
		{
			for (DoubleStat speedStat : SPEED_STATS) {
				double value = targetCreature.getTemplate().getBaseValue(speedStat).orElse(120.0) * val;
				targetCreature.getStat().addFixedValue(speedStat, value);
			}
		}
	...
	private void handleInvul(Player activeChar, Player target)
	{
		if(target.isInvulnerable())
		{
			...
			target.stopAbnormalEffect(AbnormalVisualEffect.INVINCIBILITY);
			for(Servitor servitor : target.getServitors())
			{
				servitor.getFlags().getInvulnerable().stop();
				servitor.stopAbnormalEffect(AbnormalVisualEffect.INVINCIBILITY);
			}
		...
		else
		{
			...
			target.startAbnormalEffect(AbnormalVisualEffect.INVINCIBILITY);
			for(Servitor servitor : target.getServitors())
			{
				servitor.getFlags().getInvulnerable().start();
				servitor.startAbnormalEffect(AbnormalVisualEffect.INVINCIBILITY);
			}
