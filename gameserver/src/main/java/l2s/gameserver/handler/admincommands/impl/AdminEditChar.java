...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		if(activeChar.getPlayerAccess().CanRename)
		{
			...
			if(fullString.startsWith("admin_setclass"))
				try
				{
					...
					Player player2 = target2.getPlayer();
					player2.setClassId(id, true);
			...
			if(fullString.startsWith("admin_setname"))
				try
				{
					...
					String messagePattern = "Character {} renamed to {} by GM {}";
					ParameterizedMessage message = new ParameterizedMessage(messagePattern, player, val, activeChar);
		...
		if("admin_current_player".equals(fullString))
			...
			try
			{
				...
				Player target3 = GameObjectsStorage.getPlayer(val);
		...
		else
		{
			...
			{
				GameObject target4 = activeChar.getTarget();
                if(target4 == null || !target4.isPlayer())
					return false;
                Player player3 = (Player) target4;
                player3.setRecomHave(player3.getRecomHave() + 1);
				...
				try
				{
                    ...
                    player2.setRecomHave(player2.getRecomHave() + recVal);
			...
			else if(fullString.startsWith("admin_sethero"))
			{
				...
				if(player.isCustomHero())
				{
					player.stopCustomHeroTask();
					player.stopCustomHero();
				}
				else
					player.setCustomHero(time);
			...
			else if(fullString.startsWith("admin_setsex"))
			{
				...
				try
				{
					...
					for(String value : split)
						...
						else if("sp".equalsIgnoreCase(value))
						{
							addExp = false;
							addSp = true;
						}
			...
			else if(fullString.startsWith("admin_add_bang"))
			{
				...
				int count2 = Integer.parseInt(wordList[1]);
				if(count2 < 1 || activeChar.getTarget() == null || !activeChar.getTarget().isPlayer())
				{
					activeChar.sendMessage("Usage: //add_bang count <target>");
					return false;
				}
				Player target3 = activeChar.getTarget().getPlayer();
				target3.addPcBangPoints(count2, false, true);
				activeChar.sendMessage("You have added " + count2 + " Pc Bang Points to " + target3.getName());
			}
			else if(fullString.startsWith("admin_set_bang"))
			{
				...
				Player target3 = activeChar.getTarget().getPlayer();
				target3.setPcBangPoints(count2);
				target3.sendMessage("Your Pc Bang Points count is now " + count2);
				target3.sendPacket(new ExPCCafePointInfoPacket(target3, count2, 1, 2, 12));
				activeChar.sendMessage("You have set " + target3.getName() + "'s Pc Bang Points to " + count2);
			}
            else if(fullString.startsWith("admin_setfraction"))
            {
                if(wordList.length < 1)
                {
                    activeChar.sendMessage("Usage: //setfraction fraction <target>");
                    return false;
                }
                Fraction fraction = Fraction.getIfPresent(wordList[1].toUpperCase());
                if(fraction == null || activeChar.getTarget() == null || !activeChar.getTarget().isPlayer())
                {
                    activeChar.sendMessage("Usage: //setfraction fraction <target>");
                    return false;
                }
				...
				target.changeFraction(fraction);

				target.sendMessage("Your fraction now is " + fraction);
                activeChar.sendMessage("You have set " + target.getName() + "'s fraction to " + fraction);
            }
		}
		return true;
	}
	...
	{
		List<Player> players = new ArrayList<>(GameObjectsStorage.getPlayers(player -> !player.isPhantom()));
	...
	public static void showCharacterList(Player activeChar, Player player)
	{
		...
		replyMSG.append("<tr><td width=100>Hwid:</td><td>" + player.getHwidHolder().asString() + "</td></tr>");
	...
	private void showCharacterActions(Player activeChar)
	{
		...
		if(target != null && target.isPlayer())
		{
			...
			replyMSG.append("<table width=270>");
			replyMSG.append("<tr><td width=90><button value=\"Set Fire\" action=\"bypass -h admin_setfraction fire\" width=85 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
			replyMSG.append("<td width=90><button value=\"Set Water\" action=\"bypass -h admin_setfraction water\" width=85 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td></tr>");
