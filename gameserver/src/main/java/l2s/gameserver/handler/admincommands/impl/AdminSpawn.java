...
public class AdminSpawn implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_spawn_monster:
				...
				break;
			case admin_spawn_pet:
				st = new StringTokenizer(fullString, " ");
				try
				{
					st.nextToken();
					String id = st.nextToken();
					addSummon(activeChar, Integer.parseInt(id));
				}
				catch(Exception e)
				{
					// Case of wrong monster data
	...
	private void addSummon(Player player, int summonId)
	{
		NpcTemplate summonTemplate = NpcHolder.getInstance().getTemplate(summonId);
		if(summonTemplate == null)
		{
			_log.warn("Summon: Template ID " + summonId + " is NULL FIX IT!");
			return;
		}
		...
		var skill = SkillHolder.getInstance().getSkill(1408, 2);
		final SummonInstance summon = new SummonInstance(IdFactory.getInstance().getNextId(), summonTemplate, player,
				2000000, 0, 0, 0, skill, false);
		...
		summon.setRunning();
		summon.spawnMe(Location.findAroundPosition(player, 50, 70));
