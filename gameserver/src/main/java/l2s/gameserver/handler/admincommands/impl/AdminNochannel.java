	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
				...
				else
				{
					...
					if(activeChar.getPlayerAccess().BanChatBonusId > 0 && activeChar.getPlayerAccess().BanChatBonusCount > 0)
					{
						...
						if(penaltyCount > 0)
						{
							...
							activeChar.setVar("penaltyChatCount", String.valueOf(Math.max(0, penaltyCount - add_count)), -1L);
							add_count -= penaltyCount;
						}
						if(add_count > 0)
							ItemFunctions.addItem(activeChar, activeChar.getPlayerAccess().BanChatBonusId, add_count, true);
					}
					activeChar.setVar("LastBanChatDayTime", String.valueOf(System.currentTimeMillis()), -1L);
		...
		switch(command)
		{
			...
			case admin_nc:
			{
				...
				String msg = AdminFunctions.banChat(activeChar, null, wordList[1], timeval, wordList.length > 3 ? Util.joinStrings(" ", wordList, 3) : null, false);
				activeChar.sendMessage(msg);
				if(banChatCountPerDay > -1 && msg.startsWith("\u0412\u044b \u0437\u0430\u0431\u0430\u043d\u0438\u043b\u0438 \u0447\u0430\u0442"))
				{
					++banChatCount;
					activeChar.setVar("banChatCount", String.valueOf(banChatCount), -1L);
