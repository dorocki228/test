		...
		if(activeChar.getPlayerAccess().CanBan)
			switch(command)
			{
				...
				case admin_accban:
				{
					...
					GameServer.getInstance().getAuthServerCommunication().sendPacket(new ChangeAccessLevel(account, level, banExpire));
				...
				case admin_accunban:
				{
					st.nextToken();
					String account = st.nextToken();
					GameServer.getInstance().getAuthServerCommunication().sendPacket(new ChangeAccessLevel(account, 0, 0));
					break;
				}
				case admin_trade_ban:
					return tradeBan(st, activeChar);
				...
				case admin_permaban:
					...
					GameServer.getInstance().getAuthServerCommunication().sendPacket(new ChangeAccessLevel(banaccount, -100, 0));
