...
public class AdminAttributes implements IAdminCommandHandler
{
	...
	private void setAttribute(Player activeChar, int fire, int water, int earth, int wind, int holy, int dark, int armorType)
	{
		...
		if(itemInstance != null)
		{
			...
			curFire = itemInstance.getAttributes().getValue(AttributeType.FIRE);
			curWater = itemInstance.getAttributes().getValue(AttributeType.WATER);
			curEarth = itemInstance.getAttributes().getValue(AttributeType.EARTH);
			curWind = itemInstance.getAttributes().getValue(AttributeType.WIND);
			curHoly = itemInstance.getAttributes().getValue(AttributeType.HOLY);
			curDark = itemInstance.getAttributes().getValue(AttributeType.UNHOLY);

			itemInstance.setAttributeElement(AttributeType.FIRE, fire);
			itemInstance.setAttributeElement(AttributeType.WATER, water);
			itemInstance.setAttributeElement(AttributeType.EARTH, earth);
			itemInstance.setAttributeElement(AttributeType.WIND, wind);
			itemInstance.setAttributeElement(AttributeType.HOLY, holy);
			itemInstance.setAttributeElement(AttributeType.UNHOLY, dark);
