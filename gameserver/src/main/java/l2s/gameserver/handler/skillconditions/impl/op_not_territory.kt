 ...
 * @author Java-man
 */
class op_not_territory(params: StatsSet) : SkillCondition(params) {

    private val territory = Territory()
    private val applyTo: AffectType

    init {
        val locs = params.getString("op_not_territory_param1")
        if (locs != null) {
            val polygon = Polygon()
        ...
        val applyToTemp = params.getString("op_not_territory_param2")
