...
public class WhoAmI implements IVoicedCommandHandler
{
	...
	{
		if(!Config.ALLOW_VOICED_COMMANDS || Config.WHO_AM_GM_ONLY && !player.isGM())
			return false;
		...
		double mpDrain = player.calcStat(Stats.ABSORB_DAMAGEMP_PERCENT, 0.0, target, null);
		...
		double pCritPerc = player.calcStat(Stats.P_CRITICAL_DAMAGE, target, null);
		double pCritStatic = player.calcStat(Stats.P_CRITICAL_DAMAGE_STATIC, target, null);
		double mCritPerc = player.calcStat(Stats.M_CRITICAL_DAMAGE, target, null);
		double mCritStatic = player.calcStat(Stats.M_CRITICAL_DAMAGE_STATIC, target, null);
		double blowRate = player.calcStat(Stats.FATALBLOW_RATE, target, null) * 100.0 - 100.0;
		...
		double shieldDef = shield ? player.calcStat(Stats.SHIELD_DEFENCE, player.getBaseStats().getShldDef(), target, null) : 0.0;
		...
		double critChanceResist = 100.0 - player.calcStat(Stats.P_CRIT_CHANCE_RECEPTIVE, target, null);
		double critDamResist = 100.0 - player.calcStat(Stats.P_CRIT_DAMAGE_RECEPTIVE, target, null);
