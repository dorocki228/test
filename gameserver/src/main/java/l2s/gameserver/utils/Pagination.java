...
public class Pagination<T> implements Iterable<T> {
	...
	/**
	 * Безопасный сдвиг назад, если он есть.
	 * @return
	 */
	public int prevPage() {
		return (!this.isFirst() ? this.getPreviousPage() : this.getPage());
	}

	/**
	 * Безопасный сдвиг вперёд, если он есть.
	 * @return
	 */
	public int nextPage() {
		return !this.isLast() ? this.getNextPage() : this.getPage();
	}
