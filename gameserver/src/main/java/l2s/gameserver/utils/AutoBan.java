...
public final class AutoBan
{
	...
	public static boolean isBanned(int ObjectId)
	{
		...
		{
			_log.atWarning().log( "Could not restore ban data: %s", e );
		}
		finally
		{
	...
	public static void Banned(Player actor, int period, String msg, String GM)
	{
		...
		{
			_log.atWarning().log( "Negative ban period: %s", period );
			return;
		}
		...
		{
			_log.atWarning().log( "Negative ban period | From %s to %s", date, enddate );
			return;
		}
		...
		{
			_log.atWarning().log( "could not store bans data:%s", e );
		}
		finally
		{
