...
public class SpoilTask extends PhantomAITask
{
	public SpoilTask(Player ph)
	{
		super(ph);
	}
	...
	public void runImpl()
	{
		try
		{
			...
			phantom.phantom_params.getPhantomAI().castSweperSkill();
	...
	public boolean doAction()
	{
		...
		/*		if (target != null && !target.isMinion() && target.isMonster() && !((MonsterInstance) target).isSpoiled())
				{
					castSpoilSkill(target);
				}*/
		...
		if(Math.abs(phantom.getZ() - phantom.getImpliedTargetLoc(target).getZ()) > 150)
		{
		...
		if(phantom.getDistance(target) <= 150)
		{
