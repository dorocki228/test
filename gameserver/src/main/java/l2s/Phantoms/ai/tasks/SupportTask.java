...
public class SupportTask extends PhantomAITask
{
	...
	public boolean doAction()
	{
		...
		if(target == null)
			return false;
		// if (Rnd.chance(50))
		// phantom.phantom_params.setChanceOlyCast(Rnd.get(10, 14)/10);
		switch(phantom.getPhantomType())
		{
			case PHANTOM:
			case PHANTOM_HARD:
		...
		if(Math.abs(phantom.getZ() - phantom.getImpliedTargetLoc(target).getZ()) > 200)
		{
			...
			phantom.stopMove();
			/*final SummonInstance sum = phantom.getPet();
			if (sum != null && !sum.isDead())
				sum.abortAttack(true, false);*/
			return true;
		}

		if(phantom.getCurrentMpPercents() > 12 || phantom.isMoveBlocked() || (phantom.getDistance(target) <= 900 && Rnd.chance(phantom.phantom_params.getChanceOlyCast()) ? true : !phantom.isMoving() && phantom.getDistance(target) >= 150))
		{
			phantom.phantom_params.getPhantomAI().doCast();
		}
		else if(phantom.getDistance(target) <= 150)
			phantom.getAI().Attack(target, true, false);
		else if(phantom.getDistance(target) <= 900)
