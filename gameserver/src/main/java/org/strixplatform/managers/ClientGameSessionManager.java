...
public class ClientGameSessionManager
{
	...
	// TODO[K] - maybe use concurrent set type or thread-safe?
	//private final Set<Long> SESSION_ID_LIST = Collections.synchronizedSet(new HashSet<Long>());
	...
	{
		if(MainConfig.STRIX_PLATFORM_FILES_CHECKSUM != 0)
		{
			if(clientData == null || clientData.getFilesChecksum() != MainConfig.STRIX_PLATFORM_FILES_CHECKSUM)
			{
				return false;
			}
		}
