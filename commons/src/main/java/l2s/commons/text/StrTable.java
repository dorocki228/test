...
public class StrTable
{
	...
	public String toString()
	{
		...
		for(Map.Entry<String, Integer> stringIntegerEntry : columns.entrySet())
		{
		...
		for(Map<String, String> row : rows.values())
		{
			result.append("|");
			for(Map.Entry<String, Integer> stringIntegerEntry : columns.entrySet())
				center(result, row.getOrDefault(stringIntegerEntry.getKey(), "-"), stringIntegerEntry.getValue() + 2).append("|");
			result.append("\n");
		}
		result.append(" ");
		repeat(result, "-", header.length() - 2).append(" ").append("\n");
