...
public class ClanArena extends Reflection
{
	...
	{
		@Override
		public void onCurrentHpDamage(Creature target, Creature attacker, double damage, Skill skill, boolean crit, boolean damageOverTime, boolean reflect) {
			if(!target.isNpc())
				return;
			...
			if(target == _spawnedRaidBossMain)
			{
				...
				if(phraseId > 0)
				{
					...
					if(phrase.getSize() > 0) {
						String targetName = "";
						GameObject trgt = target.getTarget();
						if(trgt != null)
							targetName = trgt.getName();
						else if(attacker != null)
							targetName = attacker.getName();
						else {
							for(Creature creature : target.getAroundCharacters(1000, 300)) {
			...
			else if(target == _spawnedRaidBossFake)
			{
				target.startAbnormalEffect(AbnormalVisualEffect.BIG_HEAD); // TODO: Должен кидаться бафф?
			}
		}

		@Override
		public void onDeath(Creature target, Creature killer)
		{
			target.removeListener(_raidListeners);
	...
	private class AiEventListener implements OnAiEventListener
	{
		...
		{
			if(evt == CtrlEvent.EVT_FINISH_WALKER_ROUTE && actor.isInZone(ARENA_ZONE_NAME))
			{
	...
	private NpcInstance spawnRaid(int npcId, boolean shuffle)
	{
		...
		if(raidNpc == null)
		{
			_log.atWarning().log( "%s: cannot spawn the Raid Boss ID[%s]!", getClass().getSimpleName(), npcId );
	...
	private void startProgressTask()
	{
		...
		{
			_log.atWarning().log( "%s: cannot start the progress task twice!", getClass().getSimpleName() );
			return;
		}
	...
	private void showTimer(Player player)
	{
		switch(_state.get())
		{
			case RAID_STATE:
			case SUPPLIES_STATE:
				player.sendPacket(new ExSendUIEvent(player, 4, 0, _totalLeftTime, 0, 0, 0, NpcString.REMAINING_TIME));
				break;
		}
	}
	...
	{
		player.sendPacket(new ExSendUIEvent(player, 1, 0, 0, 0));
	}
}