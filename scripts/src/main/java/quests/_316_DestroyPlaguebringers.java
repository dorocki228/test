...
public final class _316_DestroyPlaguebringers extends Quest
{
	...
	private static final int Varool_Foulclaws_Fang_Chance = 10;
	...
	{
		if("elliasin_q0316_04.htm".equalsIgnoreCase(event) && st.getPlayer().getRace() == Race.ELF && st.getPlayer().getLevel() >= 18)
		{
			st.setCond(1);
		}
		else if("elliasin_q0316_08.htm".equalsIgnoreCase(event))
		{
			st.finishQuest();
		}
		return event;
