...
public class _10610_TheWayToBlessingWater extends Quest{
    //NPC
    ...
    private static final int FIRE_PROOF = 75061;
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (st.getPlayer().getFraction() == Fraction.WATER) {
            int cond = st.getCond();
            if (npc.getNpcId() == QUEST_MANAGER) {
                if (cond == 0) {
                    htmltext = "qm_10610_01.htm"; //accept
                } else if (cond == 2)
                    htmltext = "qm_10610_03.htm"; //killed
//                else if(cond == 11){
//                    htmltext = "qm_10610_05.htm"; //take_cloak
//                }
            } else if (npc.getNpcId() == OREGON) {
                if (cond == 3) {
                    htmltext = "oregon_10610_01.htm"; //accept_oregon
                }
                else if (cond == 4){
                    htmltext = "oregon_10610_03.htm"; //check_items_oregon
                }
                else if (cond == 10){
                    htmltext = "oregon_10610_05.htm"; //give_adena
                }
            } else if (npc.getNpcId() == PLON){
                if (cond == 5){
                    htmltext = "plon_10610_01.htm"; //accept_plon
                }
                else if (cond == 6){
                    htmltext = "plon_10610_03.htm"; //check_items_plon
                }
                else if (cond == 7){
                    htmltext = "plon_10610_05.htm"; //accept_kill_monsters
                }
                else if (cond == 9){
                    htmltext = "plon_10610_07.htm"; //monsters_killed
                }
            }
        }
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if ("accept".equalsIgnoreCase(event)) {
            if (st.getQuestItemsCount(GLORIOUS_CLOAK) >= 1 || st.getItemEquipped(Inventory.PAPERDOLL_BACK) == GLORIOUS_CLOAK) {
                event = "qm_10610_02.htm";
                st.setCond(1);
                st.playSound(SOUND_ACCEPT);
            }
            else event = "qm_10610_nocloak.htm";
        }
        else if ("qm_10610_04.htm".equalsIgnoreCase(event)){
            st.setCond(3);
            st.playSound(SOUND_MIDDLE);
        }
        else if ("oregon_10610_02.htm".equalsIgnoreCase(event)){
            st.setCond(4);
            st.playSound(SOUND_ACCEPT);
            ...
            if (CheckIngredients(st, IngredientsOregon) && st.getQuestItemsCount(ADENA_ID) >= 1000){
                event = "oregon_10610_04.htm"; //all items
                ...
                event = "oregon_10610_notall.htm";
            }
        }
        else if ("plon_10610_02.htm".equalsIgnoreCase(event)){
            st.setCond(6);
            st.playSound(SOUND_ACCEPT);
        }
            ...
            if (CheckIngredients(st, IngredientsPlon) && st.getQuestItemsCount(ADENA_ID) >= 1000){
                event = "plon_10610_04.htm"; //all items
                ...
                event = "plon_10610_notall.htm";
            }
        }
        else if ("plon_10610_06.htm".equalsIgnoreCase(event)){
            st.playSound(SOUND_ACCEPT);
            st.setCond(8);
        }
        else if ("plon_10610_08.htm".equalsIgnoreCase(event)){
            ...
            if (st.getQuestItemsCount(ADENA_ID) >= 1000){
                ...
                st.giveItems(WATER_CLOAK, 1);
                event = "oregon_10610_06.htm";
                st.playSound(SOUND_FINISH);
                st.finishQuest();
            }
            else event = "oregon_10610_noadena.htm";
        }
//        else if (event.equalsIgnoreCase("qm_10610_06.htm")){
//            st.giveItems(WATER_CLOAK, 1);
//            st.playSound(SOUND_FINISH);
//            st.finishQuest();
//        }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.getCond() == 8){
            if (ArrayUtils.contains(WATER_MONSTERS, npc.getNpcId())){
