...
public final class _219_TestimonyOfFate extends Quest
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if("magister_kaira_q0219_05.htm".equalsIgnoreCase(event))
		{
			st.setCond(1);
			st.giveItems(KairasLetter, 1);
		}
		else if("roa_q0219_04.htm".equalsIgnoreCase(event))
		{
			st.takeItems(AldersSkull2, 1);
			st.giveItems(AldersReceipt, 1);
			st.setCond(12);
		}
		else if("magister_kaira_q0219_12.htm".equalsIgnoreCase(event))
		{
		...
		else if("arkenia_q0219_02.htm".equalsIgnoreCase(event))
		{
			st.takeItems(ThifiellsLetter, -1);
			st.giveItems(ArkeniasNote, 1);
			st.setCond(17);
		}
		else if("bloody_pixy_q0219_02.htm".equalsIgnoreCase(event))
			st.giveItems(PixyGarnet, 1);
		else if("ti_mi_riran_q0219_02.htm".equalsIgnoreCase(event))
			st.giveItems(BlightTreantSeed, 1);
		else if("arkenia_q0219_05.htm".equalsIgnoreCase(event))
		{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		else if(cond == 17)
		{
			if(st.getQuestItemsCount(PixyGarnet) > 0)
			{
				...
				{
					case Grandis:
						itemId = GrandissSkull;
						break;
					case KarulBugbear:
						itemId = KarulBugbearSkull;
						break;
					case BrekaOrcOverlord:
						itemId = BrekaOverlordSkull;
						break;
					case LetoLizardmanOverlord:
						itemId = LetoOverlordSkull;
						break;
					default:
						return null;
				}

				st.rollAndGive(itemId, 1, 1, 1, 100);
			}

			if(st.getQuestItemsCount(BlightTreantSeed) > 0)
			{
				if(npcId == BlackWillowLurker)
