...
public final class _218_TestimonyOfLife extends Quest
{
	...
	{
		if(st.getQuestItemsCount(MARK_OF_LIFE) > 0){ return COMPLETED_DIALOG; }
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		else if(cond == 15)
		{
			if(npcId == 20581 || npcId == 20582)
			{
				...
				if(st.haveQuestItem(TALINS_SPEAR_BLADE) && st.haveQuestItem(TALINS_SPEAR_SHAFT) && st.haveQuestItem(TALINS_RUBY) && st.haveQuestItem(TALINS_AQUAMARINE) && st.haveQuestItem(TALINS_AMETHYST) && st.haveQuestItem(TALINS_PERIDOT))
					st.setCond(16);
			}
		}
		return null;
