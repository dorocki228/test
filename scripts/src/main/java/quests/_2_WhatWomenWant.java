...
public final class _2_WhatWomenWant extends Quest
{
	...
	public _2_WhatWomenWant()
	{
		...
		addLevelCheck("arujien_q0002_01.htm", 2, 5);
		addRaceCheck("arujien_q0002_00.htm", Race.HUMAN, Race.ELF);
	}

	@Override
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
        if("arujien_q0002_04.htm".equalsIgnoreCase(event))
		{
			st.setCond(1);
			st.giveItems(ARUJIENS_LETTER1, 1);
		}
		else if("arujien_q0002_08.htm".equalsIgnoreCase(event))
		{
			st.setCond(4);
			st.takeItems(ARUJIENS_LETTER3, -1);
			st.giveItems(POETRY_BOOK, 1);
		}
		else if("arujien_q0002_09.htm".equalsIgnoreCase(event))
		{
			st.takeItems(ARUJIENS_LETTER3, -1);
