...
public final class _267_WrathOfVerdure extends Quest
{
	...
	{
		if("bri_mec_tran_q0267_03.htm".equalsIgnoreCase(event) && st.getPlayer().getRace() == Race.ELF && st.getPlayer().getLevel() >= 4)
		{
			st.setCond(1);
		}
		else if("bri_mec_tran_q0267_06.htm".equalsIgnoreCase(event))
		{
			st.finishQuest();
		}
		return event;
