...
public final class _232_TestOfLord extends Quest
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		if(st.getCond() == 0)
		{
			if("kakai_the_lord_of_flame_q0232_05.htm".equalsIgnoreCase(event))
			{
				st.giveItems(ORDEAL_NECKLACE, 1);
				st.setCond(1);
			}
		}
		else if("kakai_the_lord_of_flame_q0232_08.htm".equalsIgnoreCase(event))
		{
		...
		else if("atuba_chief_varkees_q0232_02.htm".equalsIgnoreCase(event))
			st.giveItems(VARKEES_CHARM, 1);
		else if("neruga_chief_tantus_q0232_02.htm".equalsIgnoreCase(event))
			st.giveItems(TANTUS_CHARM, 1);
		else if("trader_jakaron_q0232_02.htm".equalsIgnoreCase(event) && st.getQuestItemsCount(ADENA_ID) >= 1000)
		{
			st.takeItems(ADENA_ID, 1000);
			st.giveItems(NERUGA_AXE_BLADE, 1);
			st.playSound(SOUND_MIDDLE);
		}
		else if("urutu_chief_hatos_q0232_02.htm".equalsIgnoreCase(event))
			st.giveItems(HATOS_CHARM, 1);
		else if("dudamara_chief_takuna_q0232_02.htm".equalsIgnoreCase(event))
			st.giveItems(TAKUNA_CHARM, 1);
		else if("gandi_chief_chianta_q0232_02.htm".equalsIgnoreCase(event))
			st.giveItems(CHIANTA_CHARM, 1);
		else if("ancestor_martankus_q0232_04.htm".equalsIgnoreCase(event) && st.getQuestItemsCount(BEAR_FANG_NECKLACE) > 0)
		{
			st.takeItems(BEAR_FANG_NECKLACE, -1);
			st.giveItems(MARTANKUS_CHARM, 1);
			st.setCond(4);
		}
		else if("ancestor_martankus_q0232_07.htm".equalsIgnoreCase(event))
		{
			st.setCond(6);
			spawn_First_Orc(st);
		}
