...
public final class _233_TestOfWarspirit extends Quest
{
	...
	private static final int[] Noble_Ant_Drops = { KIRUNAS_THIGH_BONE, KIRUNAS_ARM_BONE, KIRUNAS_SPINE, KIRUNAS_RIB_BONE, KIRUNAS_SKULL };
	private static final int[] Leto_Lizardman_Drops = { TONARS_SKULL, TONARS_RIB_BONE, TONARS_SPINE, TONARS_ARM_BONE, TONARS_THIGH_BONE };
	private static final int[] Medusa_Drops = { HERMODTS_RIB_BONE, HERMODTS_SPINE, HERMODTS_THIGH_BONE, HERMODTS_ARM_BONE };
	...
	{
		if("seer_somak_q0233_05.htm".equalsIgnoreCase(event))
			st.setCond(1);
		else if("orim_the_shadow_q0233_04.htm".equalsIgnoreCase(event))
			st.giveItems(ORIMS_CONTRACT, 1);
		else if("seer_pekiron_q0233_02.htm".equalsIgnoreCase(event))
			st.giveItems(PEKIRONS_TOTEM, 1);
		else if("seer_manakia_q0233_02.htm".equalsIgnoreCase(event))
			st.giveItems(MANAKIAS_TOTEM, 1);
		else if("seer_racoy_q0233_02.htm".equalsIgnoreCase(event))
			st.giveItems(RACOYS_TOTEM, 1);
		else if("vivi_q0233_04.htm".equalsIgnoreCase(event))
			st.giveItems(VIVIANTES_LETTER, 1);
		else if("ancestor_martankus_q0233_03.htm".equalsIgnoreCase(event) && st.getQuestItemsCount(WARSPIRIT_TOTEM) > 0)
		{
