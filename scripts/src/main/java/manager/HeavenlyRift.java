...
{
	public static final int HEAVENLY_RIFT_DURATION = 20;
	...
	public boolean start(int delayInMinutes)
	{
		...
		_clearZoneTask = ThreadPoolManager.getInstance().schedule(this::stop, delayInMinutes, TimeUnit.MINUTES);
		return true;
	}
	...
	{
		return (int) getZone().getInsideNpcs().stream()
				.filter(npc -> npc.getNpcId() == npcId && !npc.isDead())
				.count();
	}	
