...
public class ResidenceDoor extends DoorAI
{
	...
	public void onEvtTwiceClick(Player player)
	{
		...
		int residenceId = door.getTemplate().getAIParams().getInteger("residence_id", -1);
		if(residenceId == -1)
			return;
		Residence residence = ResidenceHolder.getInstance().getResidence(residenceId);
		if(residence == null || residence.getOwner() == null
                || player.getClan() == null || player.getClan() != residence.getOwner()
                || (player.getClanPrivileges() & Clan.CP_CS_ENTRY_EXIT) != Clan.CP_CS_ENTRY_EXIT)
		{
			return;
		}
