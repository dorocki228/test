...
public class ResidenceDoor extends DoorAI
{
	...
	public void onEvtTwiceClick(final Player player)
	{
		...
		int residenceId = door.getTemplate().getAIParams().getInteger("residence_id");
		Residence residence = ResidenceHolder.getInstance().getResidence(residenceId);
		if (residence == null) {
			throw new IllegalArgumentException("Can't find residence " + residenceId + " for door " + door.getDoorId());
		}
