 ...
 * @author Bonux
**/
public class Zaken extends Fighter<NpcInstance>
{
	// Skill's
	...
	{
		return checkAbnormalTypeAndLeveL(S_ZAKEN_TRANS_DAY2NIGHT, 1);
	}

	private boolean checkAbnormalTypeAndLeveL(SkillEntry skillEntry, int abnormalLevel)
	{
		AbnormalTypeList abnormalTypeList = skillEntry.getTemplate().getAbnormalTypeList();
		return abnormalTypeList.any(abnormalType ->
				getActor().getAbnormalList().getAbnormalLevel(abnormalType) == abnormalLevel);
	}
	...
	protected void onEvtTimer(int timerId, Object arg1, Object arg2)
	{
		...
		if(timerId == 1001)
		{
			...
			{
				if(checkAbnormalTypeAndLeveL(S_ZAKEN_TRANS_DAY2NIGHT, -1))
				{
				...
				if (checkAbnormalTypeAndLeveL(S_ZAKEN_REGEN1, -1))
				{
			...
			{
				if (checkAbnormalTypeAndLeveL(S_ZAKEN_TRANS_NIGHT2DAY, 1))
				{
					addUseSkillDesire(getActor(), S_ZAKEN_TRANS_NIGHT2DAY, 1, 1, 10000000);
					_teleportCheckHpModifier = 3;
				}
				/* not exist in classic ?
				if (checkAbnormalTypeAndLeveL(S_ZAKEN_REGEN2, 11))
				{
	...
	protected void onEvtAttacked(Creature attacker, Skill skill, int damage)
	{
		...
		/*if(attacker.isMounted() && attacker.getAbnormalList().getAbnormalLevel(S_ANTI_STRIDER_SLOW.getAbnormalTypeList()) <= 0)
		{
	...
	protected void onEvtSeeSpell(Skill skill, Creature caster, Creature target)
	{
		...
		if(skill.getEffectPoint() > 0 && !skill.isBad())
		{
