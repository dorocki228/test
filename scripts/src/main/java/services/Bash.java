...
public class Bash implements IAdminCommandHandler, OnInitScriptListener
{
	...
	public void loadData()
	{
		...
		if(data == null)
		{
			_log.atInfo().log( "Service: Bash - RSS data download failed." );
			return;
		}
		...
		if(!Config.SERVICES_BASH_SKIP_DOWNLOAD)
		{
			...
			_log.atInfo().log( "Service: Bash - RSS data download completed." );
		}

		int parse = parseRSS();
		if(parse == 0)
		{
			_log.atWarning().log( "Service: Bash - RSS data parse error." );
			return;
		}
		_log.atInfo().log( "Service: Bash - RSS data parsed: loaded %s quotes.", parse );
	}

	@Override
	public void onInit()
	{
		_log.atInfo().log( "Loaded Service: Bash [%s", (Config.SERVICES_BASH_ENABLED ? "enabled]" : "disabled]") );
