...
import l2s.gameserver.network.l2.s2c.ExOlympiadMatchList.ArenaInfo;
...
public abstract class AbstractCustomObservableEvent extends SingleMatchEvent
{
	...
	private ArenaInfo _arena;
	private int _arenaId;
	private Location _observerCoords;
	...
	protected AbstractCustomObservableEvent(int id, int status, int type, String player1, String player2)
	{
		...
		AbstractCustomObservableEvent source = EventHolder.getInstance().getEvent(EventType.CUSTOM_PVP_EVENT, id);
		source.cloneTo(this);

		// копируем двери потому что в каждом новом рефлекте они свои
		List<DoorObject> doorList = source.getObjects(DOORS);
		if(!doorList.isEmpty())
			for(DoorObject door : doorList)
				addObject(DOORS, new DoorObject(door.getId()));

		List<SpawnExObject> spawnList = source.getObjects(SPAWNS);
