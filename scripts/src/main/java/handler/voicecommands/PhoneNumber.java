	...
	public boolean useVoicedCommand(String command, Player player, String args)
	{
		...
		if(args != null && !args.isEmpty())
		{
			if(!args.matches(PHONE_REGEX))
				msg = tpls.get(3);
			else if(GameServer.getInstance().getAuthServerCommunication().isShutdown())
				msg = tpls.get(4);
			else
			{
				...
				else
				{
					...
					GameServer.getInstance().getAuthServerCommunication().sendPacket(new ChangePhoneNumber(player.getAccountName(), currentNumber));
				}
			}
		}
