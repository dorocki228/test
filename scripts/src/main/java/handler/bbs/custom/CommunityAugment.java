...
public class CommunityAugment extends CustomCommunityHandler {
					...
					if (variations.isEmpty())
						...
						if(skillsVariation == null)
							...
							if(statsVariation == null) {
								content.append(tpls.get(122));
								LOGGER.atWarning().log( "%s: Cannot found stats variation for weapon TYPE[%s!", getClass().getSimpleName(), equippedWeapon.getTemplate().getWeaponFightType() );
							}
							else {
	...
	private void loadVariations(TIntObjectMap<OptionDataTemplate> variationsMap, WeaponFightType weaponFightType) {
		...
		if(skillVariations.length == 0) {
			LOGGER.atWarning().log( "%s: Skills variations is empty for weapon TYPE[%s]!", getClass().getSimpleName(), weaponFightType );
			return;
		}
		...
		if(statsVariations.length == 0) {
			LOGGER.atWarning().log( "%s: Stats variations is empty for weapon TYPE[%s]!", getClass().getSimpleName(), weaponFightType );
			return;
		}
		...
		for(int optionDataId : skillVariations) {
			...
			if(skillsOptionData == null) {
				LOGGER.atWarning().log( "%s: Cannot found option data ID[%s]!", getClass().getSimpleName(), optionDataId );
				continue;
			}

			if(skillsOptionData.getSkills().isEmpty() && skillsOptionData.getTriggerList().isEmpty()) {
				LOGGER.atWarning().log( "%s: Skills variation option data ID[%s] dont have skills and triggers!", getClass().getSimpleName(), optionDataId );
				continue;
			}
