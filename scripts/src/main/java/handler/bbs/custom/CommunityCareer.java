	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("cbbscareer".equals(cmd))
		{
			...
			if("profession".equals(cmd2))
			{
				...
				{
					if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.PROFESSION))
					{
					...
					boolean avail = false;
					ClassId classId = ClassId.valueOf(Integer.parseInt(st.nextToken()));
					for(ClassId cls : availClasses)
					{
						if(cls == classId)
						{
			...
			else if("hero".equals(cmd2))
			{
				...
				else
				{
					if(!st.hasMoreTokens()) {
						...
						String cmd3 = st.nextToken();
						if("rise".equals(cmd3)) {
							if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.LEVEL_CHANGE)) {
								onWrongCondition(player);
								return;
							}
