...
public class CommunityTeleport extends CustomCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("cbbsteleport".equals(cmd))
		{
			...
			if(info instanceof TeleportPoint)
			{
				...
				{
					if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.TELEPORT))
					{
		...
		else if("cbbsteleportpoint".equals(cmd))
		{
			...
			else
			{
				...
				if(!RequestBypassToServer.checkTeleportCond(player)) {
					player.sendMessage("Can't use right now.");
					return;
				}
		...
		else if("cbbstpsavepoint".equals(cmd))
		{
			if(BBSConfig.TELEPORT_SERVICE_FOR_PREMIUM_ONLY && !player.hasPremiumAccount())
				...
				try {
					...
					_log.atWarning().log( "CommunityTeleport: cannot save tp book mark for player %s", player.getName() );
					e.printStackTrace();
				} finally {
	...
	public void loadTeleportList()
	{
		...
		if(!file.exists())
		{
			_log.atWarning().log( "CommunityTeleport: bbs_teleports.xml file is missing." );
			return;
		}
