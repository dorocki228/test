...
public class CommunityServices extends CustomCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("cbbsservices".equals(cmd))
		{
			...
			{
				player.sendPacket(ShowBoardPacket.CLOSE);
				player.sendPacket(ExPCCafeCouponShowUI.STATIC);
				return;
			}
			...
			else if("augment".equals(cmd2))
			{
				...
				if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.AUGMENT))
				{
					onWrongCondition(player);
					return;
				}
			...
			else if("warehouse".equals(cmd2))
			{
				...
				if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.WAREHOUSE))
				{
					onWrongCondition(player);
					return;
				}
				...
				if(Config.PREMIUM_ACCOUNT_ENABLED)
				{
					if(st.hasMoreTokens())
					{
						...
						if("info".equals(cmd3))
						{
							...
							if(paTemplate == null)
							{
								_log.atWarning().log( "%s: Error while open info about premium account scheme ID[%s]! Scheme is null.", getClass().getSimpleName(), schemeId );
								return;
							}
						...
						{
							if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.PREMIUM))
							{
								onWrongCondition(player);
								return;
							}
			...
			else if("changename".equals(cmd2))
			{
				...
				if("player".equals(cmd3))
				{
					...
					{
						if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.SERVICES))
						{
							onWrongCondition(player);
							return;
						}
	...
	{
		return Util.formatDouble(value);
	}
}