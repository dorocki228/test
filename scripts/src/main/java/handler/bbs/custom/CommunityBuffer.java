...
import l2s.gameserver.data.xml.parser.CommunityBufferParser;
...
public class CommunityBuffer extends CustomCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if(bypass.startsWith("_bbsrestore"))
		{
			...
			if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && (!checkUseCondition(player, CommunityFunctionType.HEAL) || player.isInCombat()))
			{
		...
		if(bypass.startsWith("_bbscancel"))
		{
			...
			if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.CANCEL))
			{
				onWrongCondition(player);
				return;
			}
	...
	{
		if(!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.BUFFER))
		{
			onWrongCondition(player);
			return null;
		}
	...
	private static String pageMain(Player player)
	{
		...
		if(!buffSets.isEmpty())
		{
			...
			BuffSet[] buffSetsArr = buffSets.values().toArray(new BuffSet[0]);
			Arrays.sort(buffSetsArr, (s1, s2) -> Integer.compare(s2.getId(), s1.getId()));
