...
public abstract class ScriptsCommunityHandler implements IBbsHandler, OnInitScriptListener
{
	...
	protected boolean isOnPvPEvent(Player player, CommunityFunctionType functionType) {
		if(player.isInPvPEvent())
			return true;

		List<SingleMatchEvent> events = player.getEvents(SingleMatchEvent.class);
		return events.stream().anyMatch(event -> !event.canUseCommunityFunctions(player, functionType));
	}

	protected boolean checkUseCondition(Player player, CommunityFunctionType functionType)
	{
		...
		if(!BBSConfig.CAN_USE_FUNCTIONS_IN_EVENTS)	// На ивентах
			if(isOnPvPEvent(player, functionType))
				return false;
	...
	{
		if(BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player, CommunityFunctionType.ALL))
		{
