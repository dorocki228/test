...
public class OnShiftAction_NpcInstance extends ScriptOnShiftActionHandler<NpcInstance>
{
	...
	private boolean showMain(Player player, NpcInstance npc, boolean full)
	{
		...
		if(!player.getVarBoolean("noShift", false)) {
			return false;
		}
		...
		if(full)
		{
			msg.replace("%class%", npc.getClass().getSimpleName());
			msg.replace("%id%", String.valueOf(npc.getNpcId()));
			msg.replace("%respawn%", npc.getSpawn() != null ? Util.formatTime(npc.getSpawn().getRespawnDelay()) : "0");
			...
			msg.replace("%fraction%", npc.getFraction().name());
		}
	...
	public void effects(Player player, NpcInstance npc, String[] par)
	{
		...
		for(Abnormal e : npc.getAbnormalList().getEffects())
	...
	public void resists(Player player, NpcInstance npc, String[] par)
	{
        ...
        boolean hasResist = addResist(dialog, "Fire", npc.calcStat(Stats.DEFENCE_FIRE, 0, null, null));
	...
	public void aggro(Player player, NpcInstance npc, String[] par)
	{
		...
		Set<AggroList.HateInfo> set = new TreeSet<>(AggroList.HateComparator.getInstance());
