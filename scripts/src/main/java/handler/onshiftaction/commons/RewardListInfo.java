				...
				for(RewardGroup g : list)
				{
					...
					if(type == RewardType.RATED_GROUPED)
					{
						if(g.isAdena())
						{
							...
							gcount = rateAdena * npc.getStat().getValue(DoubleStat.BONUS_ADENA, 1.);
						}
						else
						{
							...
							grate = rateDrop * npc.getStat().getValue(DoubleStat.BONUS_DROP, 1.0);
					...
					{
						grate = player.getRateSpoil() * npc.getStat().getValue(DoubleStat.BONUS_SPOIL, 1.);
