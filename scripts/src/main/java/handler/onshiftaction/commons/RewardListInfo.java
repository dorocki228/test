...
public abstract class RewardListInfo
{
	...
	public static void showInfo(Player player, NpcInstance npc, RewardType showType, int page)
	{
		...
		double mod = Experience.penaltyModifier(diff);

		HashMap<Integer, String> tpls = HtmCache.getInstance().getTemplates("actions/rewardlist_info.htm", player);
		...
		{
			List<RewardList> commonRewardLists = new ArrayList<>();
			List<RewardList> spoilRewardLists = new ArrayList<>();
			List<RewardList> eventRewardLists = new ArrayList<>();
			...
			content.append(makeRewardListInfo(player, tpls, npc, commonRewardLists, RewardType.RATED_GROUPED,
					tpls.get(3), mod, showType == RewardType.RATED_GROUPED
							|| showType == RewardType.RATED_NOT_GROUPED
							|| showType == RewardType.NOT_RATED_GROUPED
							|| showType == RewardType.NOT_RATED_NOT_GROUPED, page));
	...
	public static String makeRewardListInfo(Player player, HashMap<Integer, String> tpls, NpcInstance npc, List<RewardList> rewardLists, RewardType groupType, String groupName, double penaltyMod, boolean show, int page)
	{
		if(show)
		{
			List<DropInfo> infos = new ArrayList<>();
			for(RewardList list : rewardLists)
			{
				...
				for(RewardGroup g : list)
				{
					...
					else if(type == RewardType.EVENT_GROUPED || type == RewardType.RATED_NOT_GROUPED)
					{
					...
					double itemMaxChance = (double) RewardList.MAX_CHANCE / items.size() * Config.MAX_DROP_ITEMS_FROM_ONE_GROUP / RewardList.MAX_CHANCE;
			...
			int minPage = 1;
			int maxPage = (int) Math.ceil((double) infos.size() / MAX_ITEMS_ON_PAGE);
			int currentPage = Math.max(Math.min(maxPage, page), minPage);
			...
			for(int i = ((currentPage - 1) * MAX_ITEMS_ON_PAGE); i < Math.min(currentPage * MAX_ITEMS_ON_PAGE, infos.size()); i++)
			{
				...
				dropBlock = dropBlock.replace("<?drop_max_count?>", String.valueOf(info.maxCount) + (info.maxCount < 0 ? "+" : ""));
	...
	**/
	...
	private static final IntObjectMap<BigInteger> FACTORIAL_CACHE = new CHashIntObjectMap<>();
