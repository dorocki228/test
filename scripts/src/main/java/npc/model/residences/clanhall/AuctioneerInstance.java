...
public class AuctioneerInstance extends NpcInstance
{
	...
	private static final SimpleDateFormat DATE_FORMAT_INSTANT = new SimpleDateFormat("dd/MM/yyyy HH:mm");
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if("list_all".equalsIgnoreCase(actualCommand))
		{
			...
			for(int i = min; i < max; i++)
			{
				...
				Calendar siegeDate = event.getEndSiegeDate();
		...
		else if("info".equalsIgnoreCase(actualCommand))
		{
			...
			{
				clanHall = player.getClan() == null ? null : player.getClan().getHasHideout() != 0 ? ResidenceHolder.getInstance().<AuctionClanHall> getResidence(player.getClan().getHasHideout()) : null;
			...
			Calendar c = auctionEvent == null ? Calendar.getInstance() : auctionEvent.getEndSiegeDate();
	...
	public void onMenuSelect(Player player, int ask, int reply)
	{
		if(ask == 400)
		{
			bidOnInstantClanHall(player, reply);
		}
		else if(ask == 300)
		{
			if(reply == 1)
			{
				...
				InstantClanHall rA = ResidenceHolder.getInstance().getResidence(InstantClanHall.class, 186);
				InstantClanHall rB = ResidenceHolder.getInstance().getResidence(InstantClanHall.class, 187);
				InstantClanHall rC = ResidenceHolder.getInstance().getResidence(InstantClanHall.class, 188);
				...
				{
					rA = ResidenceHolder.getInstance().getResidence(InstantClanHall.class, 188);
					rB = ResidenceHolder.getInstance().getResidence(InstantClanHall.class, 190);
					rC = ResidenceHolder.getInstance().getResidence(InstantClanHall.class, 191);
					eA = rA.getSiegeEvent();
					eB = rB.getSiegeEvent();
					eC = rC.getSiegeEvent();
				...
				if(auctionActiveA || auctionActiveB)
				{
					...
					msg.replace("<?formate_date_s?>", DATE_FORMAT_INSTANT.format(eA.getStartAuctionDate().getTime()));
					msg.replace("<?formate_date_e?>", DATE_FORMAT_INSTANT.format(eA.getEndAuctionDate().getTime()));
	...
	{
		InstantClanHall clanHall = ResidenceHolder.getInstance().getResidence(InstantClanHall.class, instantId);
