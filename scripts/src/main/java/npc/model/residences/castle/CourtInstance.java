...
import l2s.gameserver.handler.effects.impl.instant.retail.i_call_pc;
import l2s.gameserver.model.Player;
import l2s.gameserver.model.instances.NpcInstance;
import l2s.gameserver.model.pledge.Clan;
import l2s.gameserver.network.l2.components.HtmlMessage;
import l2s.gameserver.network.l2.components.SystemMsg;
import l2s.gameserver.skills.CommonSkill;
		...
		else if(condition == COND_OWNER)
		{
			...
			else if(command.startsWith("gotoleader"))
			{
				if(player.getClan() != null)
				{
					...
					{
						if (i_call_pc.Companion.checkSummonTargetStatus(player, clanLeader)) // TODO: Custom one, retail dont check it but for sure lets check same conditions like when summon player by skill.
						{
							setTarget(player);
							doCast(CommonSkill.DISPLAY_CLAN_GATE.getSkillEntry(SkillEntryType.NONE), player, false);
