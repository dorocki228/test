	...
	public void onMenuSelect(Player player, int ask, long reply, int state)
	{
		...
		else if(ask == 401)
		{
			...
			if(player.getClanId() == getOwnerId())
			{
				...
				if(skillEntry == null)
				{
					_log.atWarning().log( "Cannot use skill ID[%s] LEVEL[%s]!", skillEntry.getId(), skillEntry.getLevel() );
					return;
				}
		...
		else if(ask == 700)
		{
			...
			else if(reply == 3)
			{
				...
				if(skillEntry != null)
					...
					_log.atWarning().log( "Cannot use skill ID[%s] LEVEL[1]!", TELEPORT_SKILL_ID );
