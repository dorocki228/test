...
public class GameServerManager
{
	...
	private final Map<Integer, GameServerConnection> _gameServers = new TreeMap<>();
	private final Map<Integer, GameServerDescription> gameServerDescriptions = new TreeMap<>();
	...
	private void load()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				GameServerDescription gs = new GameServerDescription(id, rset.getString("ip"), rset.getInt("port"), rset.getString("key"));
				...
				gameServerDescriptions.put(id, gs);
			}
		}
		catch(Exception e)
		{
		...
		_log.atInfo().log( "Loaded %s registered GameServerDescription(s).", lazy(() -> _gameServers.size()) );
	}
	 ...
	 */
	public GameServerConnection[] getGameServers()
	{
		_readLock.lock();
		try
		{
			Set<GameServerConnection> gameservers = new HashSet<>(_gameServers.values());
			return gameservers.toArray(new GameServerConnection[gameservers.size()]);
		}
		finally
		{
			_readLock.unlock();
	 ...
	 */
	public int registerGameServer(HostInfo host, GameServerConnection gs)
	{
		_writeLock.lock();
		try
		{
			GameServerDescription pgs = gameServerDescriptions.get(host.getId());
			if(pgs != null)
			{
			...
			if(pgs == null || !gs.isAuthed())
			{
				if(pgs != null) {
					pgs.removeHost(host.getId());
					gs.setDescription(pgs);
				}
